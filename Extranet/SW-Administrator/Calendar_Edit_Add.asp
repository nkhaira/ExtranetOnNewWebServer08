<%
' --------------------------------------------------------------------------------------
' Add New Content Item - Include File for /SW-Administrator/Calendar_Edit.asp
'
      ...
      <TABLE WIDTH="100%" BORDER=0 BORDERCOLOR="GRAY" CELLPADDING=0 CELLSPACING=0 ALIGN="CENTER">
    	<TR>
            ...
            if Show_Content_Group then
              ...
              if (Category_Code < 8000 or Category_Code > 8999) and Admin_Access > 2 then
                ...
                response.write "<SELECT CLASS=Medium LANGUAGE=""JavaScript"" ONCHANGE=""alert('" + Translate("Important Notice",Alt_Language,conn) & "\n\n" &_
                               ...
                               "'); window.location.href='/SW-Administrator/Calendar_edit.asp?ID=add&Site_ID=" & Site_ID & "&Category_ID=" & Category_ID & "&Campaign=" & Campaign & "&Content_Group='+this.options[this.selectedIndex].value"" NAME=""Content_Group"">"
            ...
            if Content_Group > 0 and Show_Content_Group then
              ...
              if not rsCampaign.EOF then
                %>
                <SELECT CLASS=Medium LANGUAGE="JavaScript" ONCHANGE="window.location.href='/SW-Administrator/Calendar_edit.asp?ID=add&Site_ID=<%=Site_ID%>&Category_ID=<%=Category_ID%>&Content_Group=<%=Content_Group%>&Campaign='+this.options[this.selectedIndex].value" NAME="Campaign">
                <%
            ...
            <% if CInt(Category_Code) = 9 AND CInt(Site_ID) = 82 then %>
                ...
                <TD BGCOLOR="White" ALIGN=LEFT CLASS=Medium> 
                    ...
                    response.write "<OPTION CLASS=Medium VALUE="""" SELECTED>" & Translate("Select from list",Login_Language,conn) & "</OPTION>" & vbCrLf                 
                    response.write "<OPTION CLASS=Medium VALUE=""-1"">" & Translate("Yes",Login_Language,conn) & "</OPTION>" & vbCrLf
                    response.write "<OPTION CLASS=Medium VALUE=""0"">" & Translate("No",Login_Language,conn) & "</OPTION>" & vbCrLf
                    response.write "</SELECT>"
                    %>
                </TD> 
                </TR>
            ...
            if Show_Item_Number = True then
              ...
              SQLin = "SELECT MAX(Item_Number) + 1 AS Next_Number " &_
                      "FROM dbo.Calendar " &_
                     "WHERE (Item_Number not like '%[a-z]%') AND (Item_Number >= 9" & Site_ID_Pad & "0000) AND (Item_Number <= 9" & Site_ID_Pad & "9999) and site_id ="& site_ID &""                      
                ...
                <TR>
                  ...
                  <TD BGCOLOR="White" ALIGN=LEFT CLASS=Medium>                
                    <%
                        ...
                        response.write "<INPUT TYPE=""Hidden"" NAME=""hidCDN_Required"" ID=""hidCDN_Required"" VALUE=""" 
                        response.write strCDNHiddValAdd
                        response.write """>"
          ...
          %>
            <TR>
              <TD BGCOLOR="#EEEEEE" VALIGN=TOP CLASS=Medium>
            	...
            	<TD BGCOLOR="#EEEEEE" ALIGN=CENTER CLASS=Medium>
                  <!--<input type='button' CLASS='NavLeftHighlight1' onclick="Win=window.open('/sw-administrator/SW-PCAT_FNET_SAVE_ProductMultilingual.asp?ID="<%= Request.QueryString("ID") %>"&SiteId="<%= Site_ID %>"','Locale_Window','status=no,height=300,width=450,scrollbars=yes,resizable=yes,toolbar=yes,links=no,left=300,top=300');Win.focus();return false;" value='Localization' />-->
                  
                  <input type='button' CLASS='NavLeftHighlight1'  onclick="openLocalizeWindow('<%= Request.QueryString("ID") %>&SiteId=<%= Site_ID %>')" value='Localization' />
                </TD> 
              </TR>
         <% end if
         end if
            ...
            if Category_ID <> false then
                ...
                if Show_PID = true then
                  if  PID_System = 0 then  
                    ...
                    'if CInt(Site_ID) = 82  then
                    'code addedd for pricelist
                    'Biomed
                    if (CInt(Site_ID) = 82) or(CInt(Site_ID) = 3 ) or (CInt(Site_ID) = 29 ) or (CInt(Site_ID) = 46 ) or (CInt(Site_ID) = 1 ) or (CInt(Site_ID) = 4 ) or (CInt(Site_ID) = 5 ) then
                        response.write "<INPUT TYPE=""Hidden"" NAME=""Add_Edit_Flag"" VALUE=""Add"">"
                    end if  
                    '//End Code  
                  elseif PID_System = 1 then  
