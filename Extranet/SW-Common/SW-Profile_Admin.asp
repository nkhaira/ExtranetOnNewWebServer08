...
if isblank(Session("Logon_User")) or CInt(request.form("Permit_Update")) <> CInt(True) then
  ...
  SQL = ""
    ...
    if not isblank(request.form("FirstName")) then 
      SQL = SQL & ",FirstName="& "N'" & replacequote(request.form("FirstName")) & "'"
    else
    ...
    if not isblank(request.form("MiddleName")) then 
      SQL = SQL & ",MiddleName="& "N'" & replacequote(request.form("MiddleName")) & "'"         
    else
    ...
    if not isblank(request.form("LastName")) then 
      SQL = SQL & ",LastName="& "N'" & replacequote(request.form("LastName")) & "'"         
    else
    ...
    if not isblank(request.form("Suffix")) then 
      SQL = SQL & ",Suffix="& "N'" & replacequote(request.form("Suffix")) & "'"
    else
      SQL = SQL & ",Suffix=NULL"             
    end if
    ...
    if not isblank(request.form("Initials")) then 
      SQL = SQL & ",Initials="& "N'" & replacequote(request.form("Initials")) & "'"
    else
    ...
    ' Password
    Password_Flag = false
    if not isblank(request.form("Password_Current")) and not isblank(request.form("Password_Change")) and not isblank(request.form("Password_Change")) then 
      if request.form("Password_Current") = Session("Password") then
        if request.form("Password_Change") = request.form("Password_Confirm") then
          SQL = SQL & ",Password="& "N'" & replacequote(request.form("Password_Change")) & "'"
          Password_Flag = true
        end if  
      end if  
    end if
    ...
    if not isblank(request.form("Company")) then 
      SQL = SQL & ",Company="& "N'" & replacequote(request.form("Company")) & "'"
    else
    ...
    if not isblank(request.form("Company_Website")) then 
      SQL = SQL & ",Company_Website="& "N'" & killquote(request.form("Company_Website")) & "'"
    else
    ...
    if not isblank(request.form("Job_Title")) then 
      SQL = SQL & ",Job_Title="& "N'" & replacequote(request.form("Job_Title")) & "'"         
    else
    ...
    if not isblank(request.form("Business_MailStop")) then 
      SQL = SQL & ",Business_MailStop="& "N'" & replacequote(request.form("Business_MailStop")) & "'"
    else
    ...
    if not isblank(request.form("Business_Address")) then 
      SQL = SQL & ",Business_Address="& "N'" & replacequote(request.form("Business_Address")) & "'"         
    else
    ...
    if not isblank(request.form("Business_Address_2")) then 
      SQL = SQL & ",Business_Address_2="& "N'" & replacequote(request.form("Business_Address_2")) & "'"
    else
    ...
    if not isblank(request.form("Business_City")) then 
      SQL = SQL & ",Business_City="& "N'" & replacequote(request.form("Business_City")) & "'"         
    else
    ...
    if not isblank(request.form("Business_State")) or not isblank(request.form("Business_State_Other")) then 
      if not isblank(request.form("Business_State")) then
        SQL = SQL & ",Business_State="& "N'" & replacequote(request.form("Business_State")) & "'"
      else
        SQL = SQL & ",Business_State=NULL"               
      end if              
      if not isblank(request.form("Business_State_Other")) then
        SQL = SQL & ",Business_State_Other="& "N'" & replacequote(request.form("Business_State_Other")) & "'"         
      else
    ...
    if not isblank(request.form("Business_Postal_Code")) then 
      SQL = SQL & ",Business_Postal_Code="& "N'" & replacequote(request.form("Business_Postal_Code")) & "'"         
    else
    ...
    if not isblank(request.form("Business_Country")) then 
      SQL = SQL & ",Business_Country="& "N'" & replacequote(request.form("Business_Country")) & "'"         
    else
    ...
    if not isblank(request.form("Postal_Address")) then 
      SQL = SQL & ",Postal_Address="& "N'" & replacequote(request.form("Postal_Address")) & "'"
    else
    ...
    if not isblank(request.form("Postal_City")) then 
      SQL = SQL & ",Postal_City="& "N'" & replacequote(request.form("Postal_City")) & "'"
    else
    ...
    if not isblank(request.form("Postal_State")) > 0 or not isblank(request.form("Postal_State_Other")) then 
      if not isblank(request.form("Postal_State")) then
        SQL = SQL & ",Postal_State="& "N'" & replacequote(request.form("Postal_State")) & "'"
      else
        SQL = SQL & ",Postal_State=NULL"               
      end if              
      if not isblank(request.form("Postal_State_Other")) then
        SQL = SQL & ",Postal_State_Other="& "N'" & replacequote(request.form("Postal_State_Other")) & "'"         
      else
    ...
    if not isblank(request.form("Postal_Country")) then 
      SQL = SQL & ",Postal_Country="& "N'" & replacequote(request.form("Postal_Country")) & "'"         
    else
    ...
    if not isblank(request.form("Shipping_MailStop")) then 
      SQLCart = SQLCart & ",Shipping_MailStop="& "N'" & replacequote(request.form("Shipping_MailStop")) & "'"
    else
    ...
    if not isblank(request.form("Shipping_Address")) then 
      SQLCart = SQLCart & ",Shipping_Address="& "N'" & replacequote(request.form("Shipping_Address")) & "'"
    else
    ...
    if not isblank(request.form("Shipping_Address_2")) then 
      SQLCart = SQLCart & ",Shipping_Address_2="& "N'" & replacequote(request.form("Shipping_Address_2")) & "'"
    else
    ...
    if not isblank(request.form("Shipping_City")) then 
      SQLCart = SQLCart & ",Shipping_City="& "N'" & replacequote(request.form("Shipping_City")) & "'"
    else
    ...
    if not isblank(request.form("Shipping_State")) > 0 or not isblank(request.form("Shipping_State_Other")) then 
      if not isblank(request.form("Shipping_State")) then
        SQLCart = SQLCart & ",Shipping_State="& "N'" & replacequote(request.form("Shipping_State")) & "'"
      else
        SQLCart = SQLCart & ",Shipping_State=NULL"               
      end if              
      if not isblank(request.form("Shipping_State_Other")) then
        SQLCart = SQLCart & ",Shipping_State_Other="& "N'" & replacequote(request.form("Shipping_State_Other")) & "'"         
      else
    ...
    if not isblank(request.form("Shipping_Postal_Code")) then 
      SQLCart = SQLCart & ",Shipping_Postal_Code="& "N'" & replacequote(request.form("Shipping_Postal_Code")) & "'"         
    else
    ...
    if not isblank(request.form("Shipping_Country")) then 
      SQLCart = SQLCart & ",Shipping_Country="& "N'" & replacequote(request.form("Shipping_Country")) & "'"         
    else
    ...
    if not isblank(request.form("Business_Phone")) then
      if User_Region = 1 or User_Region = 3 then 
        SQL = SQL & ",Business_Phone="& "N'" & FormatPhone(replacequote(request.form("Business_Phone"))) & "'"         
      else
        SQL = SQL & ",Business_Phone="& "N'" & replacequote(request.form("Business_Phone")) & "'"         
      end if  
    else
    ...
    if not isblank(request.form("Business_Phone_Extension")) then 
      SQL = SQL & ",Business_Phone_Extension="& "N'" & replacequote(request.form("Business_Phone_Extension")) & "'"
    else
    ...
    if not isblank(request.form("Business_Phone_2")) then 
      if User_Region = 1 or User_Region = 3 then 
        SQL = SQL & ",Business_Phone_2="& "N'" & FormatPhone(replacequote(request.form("Business_Phone_2"))) & "'"
      else
        SQL = SQL & ",Business_Phone_2="& "N'" & replacequote(request.form("Business_Phone_2")) & "'"
      end if
    else
    ...
    if not isblank(request.form("Business_Phone_2_Extension")) then 
      SQL = SQL & ",Business_Phone_2_Extension="& "N'" & replacequote(request.form("Business_Phone_2_Extension")) & "'"
    else
    ...
    if not isblank(request.form("Business_Fax")) then 
      if User_Region = 1 or User_Region = 3 then
        SQL = SQL & ",Business_Fax="& "N'" & FormatPhone(replacequote(request.form("Business_Fax"))) & "'"         
      else
        SQL = SQL & ",Business_Fax="& "N'" & replacequote(request.form("Business_Fax")) & "'"         
      end if  
    else
    ...
    if not isblank(request.form("Mobile_Phone")) then 
      if User_Region = 1 or User_Region = 3 then
        SQL = SQL & ",Mobile_Phone="& "N'" & FormatPhone(replacequote(request.form("Mobile_Phone"))) & "'"         
      else
        SQL = SQL & ",Mobile_Phone="& "N'" & replacequote(request.form("Mobile_Phone")) & "'"      
      end if      
    else
    ...
    if not isblank(request.form("Pager")) then 
      if User_Region = 1 or User_Region = 3 then
        SQL = SQL & ",Pager="& "N'" & FormatPhone(replacequote(request.form("Pager"))) & "'"         
      else
        SQL = SQL & ",Pager="& "N'" & replacequote(request.form("Pager")) & "'"
      end if  
    else
      SQL = SQL & ",Pager=NULL"    
    end if
    ...
    if not isblank(request.form("Email")) then 
      SQL = SQL & ",Email="& "N'" & replacequote(request.form("Email")) & "'"         
    else
      SQL = SQL & ",Email=NULL"    
    end if
    ...
    if not isblank(request.form("Email_2")) then 
      SQL = SQL & ",Email_2="& "N'" & replacequote(request.form("Email_2")) & "'"         
    else
    ...
    if not isblank(request.form("Language")) then 
      SQL = SQL & ",Language="& "N'" & replacequote(request.form("Language")) & "'"
    else
    ...
    for aux = 0 to 9
      if not isblank(request.form("Aux_" & Trim(aux))) then 
        SQLAux = SQLAux & ",Aux_" & Trim(aux) & "="& "N'" & replacequote(request.form("Aux_" & Trim(aux))) & "'"
      else
        SQLAux = SQLAux & ",Aux_" & Trim(aux) & "=NULL"  
      end if
    next
    ...
    'response.write Session("Logon_User") & "<BR>"
    'response.write replace(SQLU,",",",<BR>")
    'response.flush
    'response.end

    conn.Execute (SQLU)   ' Update All Personal Data Fields for all Accounts

    New_Account_ID = CLng(request.form("Account_ID"))
    
    SQLAux = "UPDATE UserData SET " & Mid(SQLAux,2) & " WHERE UserData.ID=" & New_Account_ID    
    
    conn.Execute(SQLAux)  ' Update Aux Field for this site only
    ...
    for each field In rsFields.Fields
      ...
      select case UCase(field.Name)
        case "BROADCAST_DATE", "SUBSCRIPTION_DATE", "SUBSCRIPTION_FREQUENCY", _
             ...
             "SUBSCRIPTION_SECTIONS", "RTE_ENABLED", "REG_SITE_CODE", _
             "PASSWORD", "PASSWORD_CHANGE", "PASSWORD_CONFIRM"
        case else
  ...
  if CInt(Send_2_CMS_Debug) = CInt(False) then
    response.redirect BackURL & "&PF=" & CInt(Password_Flag)
  else
    response.flush
    response.end
  end if  
