...
if rsCategory("Marketing_Automation") <> null or rsCategory("Marketing_Automation") <> "" then
    ...
    if rsCategory("CDN_Implementation") <> null or rsCategory("CDN_Implementation") <> "" then
        Show_CDN_Implementation= CInt(rsCategory("CDN_Implementation"))
        Show_CDN_DefaultState= rsCategory("CDN_DefaultState")
        if Show_CDN_Implementation = True then
            response.write "<INPUT TYPE=""HIDDEN"" NAME=""Show_CDN_Implementation"" VALUE=""" & Show_CDN_Implementation & """>" 
            response.write "<INPUT TYPE=""HIDDEN"" NAME=""Show_CDN_DefaultState"" VALUE=""" & Show_CDN_DefaultState & """>" 
        end if
    end if
    ''End
    ...
    if Write_Form_Show_Values = True then
      ...
      ''Added on 11th Aug 2010, for CDN File FTP
      response.write "<INPUT TYPE=""Hidden"" NAME=""Category_Title"" VALUE=""" & rsCategory("Title") & """ >"
      ''End
