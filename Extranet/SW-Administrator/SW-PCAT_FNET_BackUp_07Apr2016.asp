    ...
    if not(objcol is nothing) then
        ...
        for icol=0 to objcol.childnodes.length-1
            ...
            if isnumeric(left(objinfo2.text,1))=false then
             ...
             'gpd RI# 1024
            if site_id = 82 then 
              Response.Write "<OPTION value=""" & objinfo1.text & """>" & objinfo2.text & "</OPTION>" & vbCrLf              
             else
...
'***********************************************************************************************************  
    ...
    .write "<TABLE cellSpacing=""1"" cellPadding=""1"" border=""0""  CLASS=NavLeftHighlight1>" & vbCrLf
    ...
    if showpcat=true then  
    if site_id = 82 then 
    .write "<SELECT  size=""0"" multiple name=""PCat_AProducts"" CLASS=Medium>" & vbCrLf
    else
    ...
    if showpcat=true then
             'if site_id=82 then  
             '.write " <SELECT LANGUAGE=""JavaScript"" multiple size=""0""  NAME=""PCat_SProducts"" CLASS=""Medium"">"
             ' else 
...
%>
    ...
    function  changeproducts(letter)
    {
        ...
        if (letter=="1")
        {
            ...
            if(Sitevariable == "82")
            { 
             FormName.PCat_SProducts.size=  FormName.PCat_SProducts.options.length;  
             FormName.PCat_AProducts.size=  FormName.PCat_AProducts.options.length; 
            }
        } 	
        else
        {
