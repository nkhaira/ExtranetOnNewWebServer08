...
Site_ID = CLng(objSite_ID_File.ReadLine)
...
CID      = CLng(request("CID"))  ' Category ID Number


if CID   = Null or CID = 0 then CID = 9000
SCID    = CLng(request("SCID")) ' Sub-Category String
...
if not isblank(request("CIN")) and isnumeric(request("CIN")) then
  CIN = CLng(request("CIN"))    ' Content Category Code
else
  CIN = 0
end if

if not isblank(request.form("CINN")) and isnumeric(request.form("CINN")) then
  CINN = CLng(request.form("CINN"))  ' Content Category ID Number
elseif not isblank(request("CINN")) and isnumeric(request("CINN")) then
  CINN = CLng(request("CINN"))  ' Content Category ID Number
else
  CINN = 0
end if
...
if not isblank(request("Show_Detail")) and isnumeric(request("Show_Detail")) then
  Show_Detail = CLng(request("Show_Detail"))  ' Current or Archive
else
  Show_Detail = -1
end if    

if not isblank(request("Show_Days")) and isnumeric(request("Show_Days")) then
  Show_Days = Abs(CLng(request("Show_Days")))
  'RI1874
  if (site_id = "11") then 
    if Show_Days < 1 or Show_Days > 60 then
      Show_Days = 7
    end if
  else
...
if not isblank(request("SortBy")) and isnumeric(request("SortBy")) then
  SortBy = CLng(request("SortBy"))' Sort By
elseif CIN > 0 then
...
do while not rsRedirect.EOF
  if CLng(CID) = CLng(rsRedirect("Button")) and not isblank(rsRedirect("Button_URL")) and rsRedirect("Button_Enable") = CLng(True) then
...
elseif not rsLogin.EOF then		
  ...
  'Modified by Zensar for RI 579

   dim strSQL, rsGetCustNumber, strBusiness, strEnabled, strCustNumber
   dim strPriceCode, rsGetPriceCode, strPriceURL
      strBusiness=rsLogin("Business_System")
	

	''' Check whether Business System is ORA '''
      If Trim(strBusiness)="ORA" then
	
	''' Check to see that Customer Number is not null 

		strCustNumber=rsLogin("Fluke_ID")

		if Trim(strCustNumber) <> "" then
		''' Check whether User is enabled for Price List Access Flag 
			''strEnabled=rsLogin("PriceListFlag")
			
			''if strEnabled=-1 then
				'' Get the Price List
				set rsGetPriceCode=Server.CreateObject("Adodb.Recordset")
				strPriceURL="select PriceListCode from Customer_PriceLists where CustomerNumber='" & strCustNumber & "' and Site_ID=" & Site_ID & ""
				rsGetPriceCode.Open strPriceURL, conn, 3, 3
				
				if not rsGetPriceCode.eof then
					Session("PriceListCode")=rsGetPriceCode("PriceListCode")
				end if
			''end if
		end if
	end if
...
'        response.write Session("Pricelistcode")
  ...
  if Shopping_Cart = CLng(True) then
  ...
  if isdate(rsLogin("ExpirationDate")) then
    ...
    if rsLogin("ExpirationDate") <= Date then
      ...
      'RI# 2219
        Session("ErrorString") = Session("ErrorString") & "<A HREF="""& Server.HTMLEncode("mailto:"& AA_Email &"?Subject=Account%20Expiration%20-%20User%20Re-Instatement%20Request&body=Site:%20" & Replace(Replace(Site_Description,"Partner Portal - ","")," ","%20") & "%0AID:%20" & Login_ID & "%0D%0AName:%20" & Login_FirstName & "%20" & Login_LastName & "%0D%0ACompany:%20"+Replace(Login_Company," ","%20")) & """>" & AA_Name & "</A>  " & Translate("by clicking on the Account Administrator&acute;s name.",login_language,conn)
      end if  
      response.redirect "/register/default.asp?backurl=" & BackURL  
    end if
  end if          
  
  if CLng(rsLogin("FCM")) = true then
    Access_Level = 1
  ...
  if Access_Level >= 2 then
    Session.Timeout = CLng(Site_Timeout) * 2
  else
  ...
  if not rsNavigation.EOF then 
  	do while not rsNavigation.eof
  	  i = i + 1
  	  if  CLng(rsNavigation("Button_Enable")) = False  or isblank(rsNavigation("Button"))  or isblank(rsNavigation("Button_ENG")) then
  	    Button(i)       = ""
  	    Button_Title(i) = ""
  	    Button_Help(i)  = ""
  	  elseif i = 29 then
  ...
  if CLng(Shopping_Cart) = CLng(True) then
    if CLng(Session("Cart_Active")) = -2 then   ' Initally set in Global.ASA - Do once for Session
      ...
      if rsItems("Items") > 0 then
        Session("Cart_Active") = CLng(True)
      else
        Session("Cart_Active") = CLng(False)
      end if
      rsItems.close
      set rsItems = nothing
    end if
  ...
  if CID = Button(0) then
    ...
    SQL = "SELECT Text_Home_Page.Text_Region_" & Trim(CStr(Login_Region)) & " FROM Text_Home_Page WHERE Text_Home_Page.Site_ID=" & CLng(Site_ID)
    ...
    SQL = "SELECT Calendar_Category.* FROM Calendar_Category WHERE Calendar_Category.Site_ID=" & CLng(Site_ID) & " AND Calendar_Category.Code=9999"
  ...
  elseif CID = Button(5) then
        ...
        SQL = SQL & "WHERE Site_Aux.Site_ID=" & Site_ID & " AND Site.Enabled=" & CLng(True) & " ORDER BY Site.Site_Description"
        ...
        if not rsSite_Aux.EOF then
          ...
          do while not rsSite_Aux.EOF
            ...
            Select_Site_ID = rsSite_Aux("ID")
            
            response.write "<TR>"
            if Select_Site_ID = "11" then
            
				response.Write "<TD CLASS=NavLeftHighlight1>"
				response.write "<A HREF=""http://us.flukecal.com/support/my-met-support"" CLASS=NavLeftHighlight1>"
				' Use Text Link
