  ...
  case "update"
    ...
    if not isblank(request("Quantity")) and isnumeric(request("Quantity")) then
      ...
      if request("Quantity") > 0 then
        ...
        if Max_Limit = 0 then Max_Limit = 50
  ...
  case "transfer"
    ...
    if Review = True and isblank(ErrMessage) then
      ...
      for stc = 0 to Ship_To_Count
        ...
        'Modified for consuming the new NAC web service 03/01/2009 as we are not going to use the DCG system anymore.
        select case strMyEnvironment
	          ...
	          case "TEST"                    
               strRemoteHostURL = "http://" & strRemoteHostName & strRemoteHostTargetFile             
	          case else
               strRemoteHostURL = "https://" & strRemoteHostName & strRemoteHostTargetFile            
        end select
        ...
        if not Script_Debug then
          ...
          select case strMyEnvironment
			...
			HTTPRequest.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
	          ...
	          case "TEST"                    
               		HTTPRequest.Open "GET", strRemoteHostURL & "?" & strPost_QueryString , False              
			HTTPRequest.setRequestHeader "Content-Type", "application/x-www-form-urlencoded"
          		on error resume next          
          		HTTPRequest.Send 
	          case else
...
'          Mailer.AddRecipient "Kelly.Whitlock", "Kelly.Whitlock@Fluke.com"
          ...
          PostErrMessage = "Order Post Error: " & strRemoteHostURL & bResponse & vbCrLf &_
                           "Calling Script: " & request.ServerVariables("Script_Name") & vbCrLf & vbCrLf &_
                           "The following debugging information was returned by the receiving Literature Order System:" & vbCrLf & vbCrLf &_
                           strResponse & vbCrLf
...
sub Shopping_Cart_Data
  ...
  do while not rsCart.EOF
    ...
    if Action_Sequence <> "transfer" and Action_Sequence <> "review" then
      ...
      'code to render maximum quatity, added by Zensar on March 16 2009
      'Modified by zensar on September 28 2009 for changing the max quantity to 50 instead of 1.
      if cint(rsCart("Max_Limit"))=0 then
         Response.Write Translate("<b>Max. Quantity:",Login_Language,conn) & "&nbsp; 50</b>" 
      else
         Response.Write Translate("<b>Max. Quantity:",Login_Language,conn) & "&nbsp;" & rsCart("Max_Limit")  &"</b>" 
      end if
  ...
  function CheckRequiredFields() {
    ...
    if (ErrorMsg.length) {
      ...
      if (dNT.all || dNT.getElementById) {
        ...
        setTimeout('alert("<%=Translate("Your literature order has been submitted.",Alt_Language,conn)%>")', 1000);
      }
      return(true);
    }
//-->
