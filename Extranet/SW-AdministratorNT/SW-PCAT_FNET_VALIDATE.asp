...
if clng(request.form("PCat"))>= 0 then
	if (trim(request.form("opr"))="U" )   then 
			Dim objCommand	
			Dim rsResult
			Dim strResult
			Dim arrResult
			...
			if not(objcol is nothing) then
			    ...
			    elseif trim(objcol.text)="SUBTYPE" then
					errorText = errorText & Translate("Product Subtype not available in PCat." & vbCrLf,Login_Language,conn) 
