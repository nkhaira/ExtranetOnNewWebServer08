...
if Site_ID = 100 then
  ...
  %>
    <SELECT  Class=Medium LANGUAGE="JavaScript" ONCHANGE="siteSelectOnChange();" NAME="Site_ID" ID="Site_ID">
  <%
  ...
  if request("ID") = "edit_category" then
	...
	'Validation to check max length on description text area starts
        ...
        ''Currentlly it is only for Fnet
         ...
         'RI#1705 change -gpd
         'Biomed
        'if Site_ID=82 or (CInt(Site_ID) = 3 and CInt(request("Category_ID")) =117)  then
        if Site_ID=82 or (CInt(Site_ID) = 3)  or (CInt(Site_ID) = 46 and CInt(request("Category_ID")) =989) or Site_ID=29 OR Site_ID=46 OR Site_ID=1 OR Site_ID=4 OR Site_ID=5  then
            response.write "<TR><TD COLSPAN=2 BGCOLOR=""#666666"" Class=Medium></TD></TR>" & vbCrLf
...
{	
	...
	{
		window.location.href="default.asp?Site_ID=" + siteId;
	}
}
