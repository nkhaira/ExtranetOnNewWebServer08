...
<table  width="700"  cellpadding="0">
        <tr>
            <td><b>File FTP Utility (FCal - Pressure, Flow) </b>
            </td>
        </tr>
...
if ((isSubmit = 1 and len(strSelectedCat) > 0) OR (strRequestID <> "" OR strRequestID <> null)) then
    ...
    if (isSubmit = 1 and len(strSelectedCat) > 0) then
        ...
        'SQL = SQL & " WHERE Site_ID = 5  AND DATEDIFF(day,Udate, getdate()) <= 2 "
        SQL = SQL & " WHERE Site_ID = 5  "
    ...
    Do While not rsAssets.EOF 
      ...
      strFTPFilePath = ""
       strFTPFilePath = FTPFile(Server.MapPath("/DHI-Sales/" & rsAssets("File_Name")), rsAssets("File_Name")) ''FTP the file
      ...
      'if isnull(rsAssets("CDNFilePath")) OR rsAssets("CDNFilePath") = "" then
           ...
           ''3.2 FTP the file
           ''if (ValidateFTPFileName("DHI-Sales/" & rsAssets("File_Name"),rsAssets("ID"))) Then
...
function FTPFile(sSourceFile, sFTPLocation)
    ...
    'File re-naming ------------------------------
    Dim fsoRenameFcalPressure_FileServerFile
    Set fsoRenameFcalPressure_FileServerFile = CreateObject("Scripting.FileSystemObject")
    
    Dim FcalPressureFleNameBeforeModification : FcalPressureFleNameBeforeModification  =  fsoRenameFcalPressure_FileServerFile.getfilename(sSourceFile)
    Dim FcalPressureFileNameAfterModification : FcalPressureFileNameAfterModification = "AmsToCDNUploaded" & FcalPressureFleNameBeforeModification
    
    Dim FcalPressureCdnUploadedFilePath : FcalPressureCdnUploadedFilePath = Replace(sSourceFile,FcalPressureFleNameBeforeModification, FcalPressureFileNameAfterModification)
    
    if fsoRenameFcalPressure_FileServerFile.FileExists(sSourceFile) Then
        if fsoRenameFcalPressure_FileServerFile.FileExists(FcalPressureCdnUploadedFilePath) then
            on error resume next
            fsoRenameFcalPressure_FileServerFile.DeleteFile FcalPressureCdnUploadedFilePath, True ' Delete Record, True=Read Only and non-Read Only
            on error goto 0
        end if 
                              
        fsoRenameFcalPressure_FileServerFile.MoveFile sSourceFile,FcalPressureCdnUploadedFilePath
     end if 
    '---------------------------------------------
...
function ValidateFTPFileName(sFileNameToFTP, sID)
   ...
   ' sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=" & CInt(Site_ID) & " AND CDNFilePath = '" & sFileNameToFTP & "'"
    sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=5 AND CDNFilePath = '" & sFileNameToFTP & "'"
...
function UpdateCalendarFTPDetail(sID, sCDNRequried, sFTPLocation)
    Dim sSQL
    sSQL = "UPDATE Calendar SET CDN_Required = -1,CDNFilePath='" & sFTPLocation & "' WHERE Site_ID=5 AND Id = " & sID 
...
function DisplayCategories() 
	...
	sqlCategory = "SELECT ID, Title  from Calendar_Category Where Site_Id = 5 AND CDN_Implementation = -1 " 
