...
if Script_Debug = false then
 ...
 ' Dim GroupsShortDesc
  ...
  'GroupsShortDesc= rsSite("Site_Short_Descr")
  ...
  with msg 
    .Fields.Item("urn:schemas:mailheader:X-Priority") = 3
    .Fields.Update
  end with

  if Site_ID = 10 or Site_ID = 1 then
	  'Mailer.FromName       ="Fluke Calibration Partner Portal - Electrical - RF - Power/Energy"
    fromName = "Fluke Calibration Partner Portal - Electrical - RF - Power/Energy"
  elseif Site_ID=5 then
	  'Mailer.FromName       ="Fluke Calibration Partner Portal - Pressure - Flow"
    fromName = "Fluke Calibration Partner Portal - Pressure - Flow"
  else
  ...
  msg.From = """" & fromName & """" & MailFromAddress
  msg.Sender = """" & fromName & """" & MailFromAddress
  ...
  If Send_Email_Admin = true then
    ...
    if not isblank(msg.To) then
      msg.To = msg.To & ";" & """" & MailReplyToName & """" & MailReplyTo
    else
...
''' Check for user's record in Activity table - RI 648 '''''
	 Set rsActivity = Server.CreateObject("ADODB.Recordset")
	SQL_Activity=""
	SQL_Activity="select Activity.* from Activity where Activity.Account_ID=" & Account_Id & "  AND Activity.Site_ID=" & Site_ID &""	
    	 rsActivity.Open SQL_Activity, conn, 3, 3
	if rsActivity.eof then 
      ...
      and (Admin_Account_Region = 4 or Admin_Account_Region = rsUser("Region")) then   ' Check to see that User is not an Admin Account
          ...
          if Send_Email_Fcm = true then
            ...
            if not rsManager.EOF then
              ...
              'Mailer.AddCC ManagerName, rsManager("EMail")
               if not isblank(msg.Cc) then
                msg.Cc = msg.Cc & ";" & """" & ManagerName & """" & rsManager("EMail")
              else
                msg.Cc = """" & ManagerName & """" & rsManager("EMail")
              end if
            end if
        ...
        ErrorMessage = ErrorMessage & "<LI>" & Translate("This account cannot be deleted as it has an Asset Activity associated with it.",Login_Language,conn) & "</LI>"
	end if
	rsActivity.close()
    end if
 ...
 if (request("Business_System")="ORA") then
		if not isblank(request("Fluke_ID")) then		
          		 sqlf = sqlf & ",PriceListFlag"
        		 sql  = sql  & "," & CInt(True)     
        		 sqlu = sqlu & ",PriceListFlag=" & CInt(True)
	  	 else
      		  sqlf = sqlf & ",PriceListFlag"
		        sql  = sql  & "," & CInt(False)
		        sqlu = sqlu & ",PriceListFlag=" & CInt(False)    
	   end if
      else
        sqlf = sqlf & ",PriceListFlag"
        sql  = sql  & "," & CInt(False)
        ...
        if site_id =3 then
			'---------------Modified by Zensar for implementing Field Marketing Site changes--------
					...
					if Request("CFMEnabled")= 1 then
								if trim(request("ExpirationDate")) <> "" then
...
cmd.Parameters.Append cmd.CreateParameter("@AccountID",adBigInt,adParamInput,0)
