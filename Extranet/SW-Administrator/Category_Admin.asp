...
'if CInt(request("Site_ID")) = 82 then
'added for pricelist CDN
'RI#1705 -gpd
'Biomed
'if ((CInt(request("Site_ID")) = 82) OR (CInt(request("Site_ID")) = 3))  then
if ((CInt(request("Site_ID")) = 82) OR (CInt(request("Site_ID")) = 3)OR (CInt(request("Site_ID")) = 29) OR (CInt(request("Site_ID")) = 46) OR (CInt(request("Site_ID")) = 1) OR (CInt(request("Site_ID")) = 4) OR (CInt(request("Site_ID")) = 5) )  then
    if request("Enable_CDN") = "on" then           
      SQL = SQL & ",Calendar_Category.CDN_Implementation=" & CInt(True)
      if request("CDN_State") <> "" then
        SQL = SQL & ",Calendar_Category.CDN_DefaultState='" & request("CDN_State") & "'"
      else
        SQL = SQL & ",Calendar_Category.CDN_DefaultState='UncheckedEnabled'"  ''Default value
      end if 
    else
      SQL = SQL & ",Calendar_Category.CDN_Implementation=" & CInt(False)
      SQL = SQL & ",Calendar_Category.CDN_DefaultState=Null" 
    end if  
end if  
''End
