...
<table  width="700"  cellpadding="0">
        <tr>
            <td><b>File FTP Utility (Bio-Medical)</b>
            </td>
        </tr>
...
if ((isSubmit = 1 and len(strSelectedCat) > 0) OR (strRequestID <> "" OR strRequestID <> null)) then
    ...
    if (isSubmit = 1 and len(strSelectedCat) > 0) then
        ...
        'SQL = SQL & " WHERE Site_ID = 46  AND DATEDIFF(day,Udate, getdate()) <= 2 "
        SQL = SQL & " WHERE Site_ID = 46  "
    ...
    Do While not rsAssets.EOF 
      ...
      strFTPFilePath = ""
       strFTPFilePath = FTPFile(Server.MapPath("/Biomedical/" & rsAssets("File_Name")), rsAssets("File_Name")) ''FTP the file
...
function FTPFile(sSourceFile, sFTPLocation)
    ...
    'File re-naming ------------------------------
    Dim fsoRenameBioMed_FileServerFile
    Set fsoRenameBioMed_FileServerFile = CreateObject("Scripting.FileSystemObject")
    
    Dim BioMedFleNameBeforeModification : BioMedFleNameBeforeModification  =  fsoRenameBioMed_FileServerFile.getfilename(sSourceFile)
    Dim BioMedFileNameAfterModification : BioMedFileNameAfterModification = "AmsToCDNUploaded" & BioMedFleNameBeforeModification
    
    Dim BioMedCdnUploadedFilePath : BioMedCdnUploadedFilePath = Replace(sSourceFile,BioMedFleNameBeforeModification, BioMedFileNameAfterModification)
    
    if fsoRenameBioMed_FileServerFile.FileExists(sSourceFile) Then
        if fsoRenameBioMed_FileServerFile.FileExists(BioMedCdnUploadedFilePath) then
            on error resume next
            fsoRenameBioMed_FileServerFile.DeleteFile BioMedCdnUploadedFilePath, True ' Delete Record, True=Read Only and non-Read Only
            on error goto 0
        end if 
                              
        fsoRenameBioMed_FileServerFile.MoveFile sSourceFile,BioMedCdnUploadedFilePath
     end if 
    '---------------------------------------------
...
function ValidateFTPFileName(sFileNameToFTP, sID)
   ...
   ' sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=" & CInt(Site_ID) & " AND CDNFilePath = '" & sFileNameToFTP & "'"
    sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=46 AND CDNFilePath = '" & sFileNameToFTP & "'"
...
function UpdateCalendarFTPDetail(sID, sCDNRequried, sFTPLocation)
    Dim sSQL
    sSQL = "UPDATE Calendar SET CDN_Required = -1,CDNFilePath='" & sFTPLocation & "' WHERE Site_ID=46 AND Id = " & sID 
...
function DisplayCategories() 
	...
	sqlCategory = "SELECT ID, Title  from Calendar_Category Where Site_Id = 46 AND CDN_Implementation = -1 " 
