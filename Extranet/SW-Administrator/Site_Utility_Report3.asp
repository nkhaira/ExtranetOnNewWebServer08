...
response.write Summary_Class
  ...
  elseif Utility_ID = 72 and Admin_Access <> 3 then
    ...
    SQL = "SELECT    dbo.Activity.Account_ID, dbo.Activity.Site_ID, dbo.Activity.Calendar_ID, dbo.Calendar.Item_Number, dbo.Calendar.Revision_Code, dbo.Calendar.Product, dbo.Calendar.Title, dbo.Calendar.Sub_Category, dbo.Calendar.Language, dbo.Activity.View_Time, " &_
    ...
    if not rsActivity.EOF then
      ...
      do while not rsActivity.EOF
        ...
        if temp_old <> temp_new then
          rsActivity.MovePrevious
		'response.write "Account id is " & rsActivity("Account_ID")
          response.write "<TR>" & vbCrLf
...
if Category_Code <> "" then
	Category_Code=Server.URLEncode("Category_Code")
else
	Category_Code=""
end if
if rsActivity("CMS_Path") <> "" then
	Site_Path=Server.URLEncode("CMS_Path")
else
	Site_Path=""
end if
if CInt(Group_By) <> 1 then

		url="AssetActivityDetail.asp?wsite=1&id=" & rsActivity("Calendar_ID") & "&Begin_Date=" & Begin_Date & "&Interval=" & Interval & "&Site_ID=" & Site_ID & "&Category_Code=" & Category_Code & "&Country_Code="& Country_Code & "&Local_Code="& Local_Code & "&Item_Number="& rsActivity("Item_Number") & "&Site_Path="& Site_Path
else
		url="AssetActivityDetail.asp?wsite=1&id=" & rsActivity("Calendar_ID") & "&Begin_Date=" & Begin_Date & "&Interval=" & Interval & "&Site_ID=" & Site_ID & "&Category_Code=" & Category_Code & "&Country_Code="& Country_Code & "&Local_Code="& Local_Code & "&Item_Number="& rsActivity("Item_Number") & ""
end if
	''response.write url
		if (rsActivity("Account_ID") <> 1) then
	          response.write "<TD CLASS=Small BGCOLOR=""#FFFFFF"" ALIGN=""RIGHT""><a href=" & url & ">" & FormatNumber(temp_cnt,0) & "</a></TD>" & vbCrLf
		else
      ...
      if temp_cnt >= 1 then
        rsActivity.MovePrevious
	'response.write "Account id is searc" & rsActivity("Account_ID")
          response.write "<TR>" & vbCrLf
          response.write "<TD CLASS=Small BGCOLOR=""#FFFFFF"" ALIGN=""RIGHT"">" & rsActivity("Item_Number") & "</TD>" & vbCrLf
          response.write "<TD CLASS=Small BGCOLOR=""#FFFFFF"" ALIGN=""CENTER"">" & rsActivity("Revision_Code") & "</TD>" & vbCrLf          
          response.write "<TD CLASS=Small BGCOLOR=""#FFFFFF"" ALIGN=""RIGHT"">" & rsActivity("Calendar_ID") & "</TD>" & vbCrLf
...
if Group_By <> 1 then
		url="AssetActivityDetail.asp?wsite=1&id=" & rsActivity("Calendar_ID") & "&Begin_Date=" & Begin_Date & "&Interval=" & Interval & "&Site_ID=" & Site_ID & "&Category_Code=" & Server.URLEncode(Category_Code) & "&Country_Code="& Country_Code & "&Local_Code="& Local_Code & "&Item_Number="& rsActivity("Item_Number") & "&Site_Path="& Server.URLEncode(rsActivity("CMS_Path")) & ""
else
		url="AssetActivityDetail.asp?wsite=1&id=" & rsActivity("Calendar_ID") & "&Begin_Date=" & Begin_Date & "&Interval=" & Interval & "&Site_ID=" & Site_ID & "&Category_Code=" & Server.URLEncode(Category_Code) & "&Country_Code="& Country_Code & "&Local_Code="& Local_Code & "&Item_Number="& rsActivity("Item_Number") & ""
end if
if (rsActivity("Account_ID") <> 1) then
