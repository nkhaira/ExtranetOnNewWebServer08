...
elseif not isblank(oFileUpEE.form("LOGON_USER")) then
  Admin_Name = oFileUpEE.form("LOGON_USER")
else
  Admin_Name = ""
end if
...
elseif not isblank(oFileUpEE.form("LOGON_Password")) then
  Admin_Password = oFileUpEE.form("LOGON_Password")
elseif not isblank(oFileUpEE.form("Password")) then
  Admin_Password = oFileUpEE.form("Password")
else
  Admin_Password = ""
end if
...
elseif not isblank(oFileUpEE.form("Site_ID")) and isnumeric(oFileUpEE.form("Site_ID")) then
...
if Site_ID = 100 or Site_ID = 1001 then
  ...
  if not rsLogin.EOF then
    ...
    if Admin_Access > 0 then
      ...
      if isblank(Session("Language")) and isblank(Request.QueryString("Language")) then
