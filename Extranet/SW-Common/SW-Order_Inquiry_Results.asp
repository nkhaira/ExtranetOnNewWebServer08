...
Screen_Title    = Translate(Site_Description,Alt_Language,conn) & " - " & Translate("Order Status Information",Alt_Language,conn)
...
with response
  ...
  .write "LANGUAGE=""Javascript"" ONCLICK=""location.href='/SW-Common/SW-Order_Inquiry_Redirect.asp'; return false;"" TITLE=""Return to Order Inquiry Select"" onmouseover=""this.className='NavLeftButtonHover'"" onmouseout=""this.className='Navlefthighlight1'"">"
  ...
  .write "&nbsp;&nbsp;"  
  .write "<INPUT CLASS=NavLeftHighlight1 TYPE=""BUTTON"" NAME=""New_Search"" VALUE=""" & " " & Translate("New Search",Login_Language,conn) & """ "
  .write "LANGUAGE=""Javascript"" ONCLICK=""javascript:document.foodle.submit();"" TITLE=""New Search"" onmouseover=""this.className='NavLeftButtonHover'"" onmouseout=""this.className='Navlefthighlight1'"">"
...
<script language="Javascript">
	function Track(num,carrier) {
        ...
        // RI : 1675 (FOR IT WEB)Incorrect FEDEX tracking link in Order display in Partner Portal
        // Changed By : nkulkarn On 06/09/2011
        
        	var vHref = 'https://www.fedex.com/Tracking?action=track&language=english&';
        vHref += 'cntry_code=us&initial=x&tracknumbers=' + num;
			...
			do until dbRS.EOF
				...
				if len(waybill) > 0 then
					if Instr(lcase(waybill),"b/o") > 0 then
						waybill = "back order"
						shp_method = "&nbsp;"
				    'Commented out by zensar.For UPS all waybill numbers start with 1Z. 		
					'elseif (Instr(ucase(shp_method),"UPS") = 1) then
					elseif left(ucase(trim(waybill)),2) = "1Z" then
						bShowWayUL = True
				...
				with response
					...
					.write ">" & CkBlank(dbRS("Quantity_Cancelled"))
  					.write "</td>" & vbcrlf
  					.write "        <td nowrap class=""Small"" BGCOLOR=" & Shading & ">"
					'Modified by zensar on 9/12/2008 for showing text as "TBA" in case the date diff
					'is more then 100.
					if isnull(dbRS("Scheduled_Ship_Date"))=false then
						if datediff("d",now,dbRS("Scheduled_Ship_Date")) > 100 then
							.write Translate("TBA",Alt_Language,conn)
						else
...
sub Table_Begin()
    ...
    response.write "        <TD VALIGN=""top"" WIDTH=""100%"" CLASS=NavBorder>" & vbCrLf
end sub      
...
sub Nav_Border_Begin()
    ...
    response.write "      <TR>" & vbCrLf
    response.write "        <TD BACKGROUND=""/images/SideNav_TL_corner.gif""><IMG SRC=""/SideNav_TL_corner.gif"" BORDER=""0"" WIDTH=""8"" HEIGHT=""6"" ALT=""""></TD>" & vbCrLf
    response.write "        <TD><IMG SRC=""/images/Spacer.gif"" BORDER=""0"" HEIGHT=""6"" ALT=""""></TD>" & vbCrLf
    response.write "        <TD BACKGROUND=""/images/SideNav_TR_corner.gif""><IMG SRC=""/images/Spacer.gif"" BORDER=""0"" WIDTH=""8"" HEIGHT=""6"" ALT=""""></TD>" & vbCrLf
    response.write "      </TR>" & vbCrLr
    response.write "      <TR>" & vbCrLf
