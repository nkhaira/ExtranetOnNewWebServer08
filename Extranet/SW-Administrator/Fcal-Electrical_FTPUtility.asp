...
<table  width="700"  cellpadding="0">
        <tr>
            <td><b>File FTP Utility (FCal - Electrical) </b>
            </td>
        </tr>
...
if ((isSubmit = 1 and len(strSelectedCat) > 0) OR (strRequestID <> "" OR strRequestID <> null)) then
    ...
    if (isSubmit = 1 and len(strSelectedCat) > 0) then
        ...
        'SQL = SQL & " WHERE Site_ID = 1  AND DATEDIFF(day,Udate, getdate()) <= 2 "
        SQL = SQL & " WHERE Site_ID = 1  "
    ...
    Do While not rsAssets.EOF 
      ...
      strFTPFilePath = ""
       strFTPFilePath = FTPFile(Server.MapPath("/Calibration-Sales/" & rsAssets("File_Name")), rsAssets("File_Name")) ''FTP the file
      ...
      'if isnull(rsAssets("CDNFilePath")) OR rsAssets("CDNFilePath") = "" then
           ...
           ''3.2 FTP the file
           ''if (ValidateFTPFileName("Calibration-Sales/" & rsAssets("File_Name"),rsAssets("ID"))) Then
...
function FTPFile(sSourceFile, sFTPLocation)
    ...
    'File re-naming ------------------------------
    Dim fsoRenameFcalElectrical_FileServerFile
    Set fsoRenameFcalElectrical_FileServerFile = CreateObject("Scripting.FileSystemObject")
    
    Dim FcalElectricalFleNameBeforeModification : FcalElectricalFleNameBeforeModification  =  fsoRenameFcalElectrical_FileServerFile.getfilename(sSourceFile)
    Dim FcalElectricalFileNameAfterModification : FcalElectricalFileNameAfterModification = "AmsToCDNUploaded" & FcalElectricalFleNameBeforeModification
    
    Dim FcalElectricalCdnUploadedFilePath : FcalElectricalCdnUploadedFilePath = Replace(sSourceFile,FcalElectricalFleNameBeforeModification, FcalElectricalFileNameAfterModification)
    
    if fsoRenameFcalElectrical_FileServerFile.FileExists(sSourceFile) Then
        if fsoRenameFcalElectrical_FileServerFile.FileExists(FcalElectricalCdnUploadedFilePath) then
            on error resume next
            fsoRenameFcalElectrical_FileServerFile.DeleteFile FcalElectricalCdnUploadedFilePath, True ' Delete Record, True=Read Only and non-Read Only
            on error goto 0
        end if 
                              
        fsoRenameFcalElectrical_FileServerFile.MoveFile sSourceFile,FcalElectricalCdnUploadedFilePath
     end if 
    '---------------------------------------------
...
function ValidateFTPFileName(sFileNameToFTP, sID)
   ...
   ' sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=" & CInt(Site_ID) & " AND CDNFilePath = '" & sFileNameToFTP & "'"
    sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=1 AND CDNFilePath = '" & sFileNameToFTP & "'"
...
function UpdateCalendarFTPDetail(sID, sCDNRequried, sFTPLocation)
    Dim sSQL
    sSQL = "UPDATE Calendar SET CDN_Required = -1,CDNFilePath='" & sFTPLocation & "' WHERE Site_ID=1 AND Id = " & sID 
...
function DisplayCategories() 
	...
	sqlCategory = "SELECT ID, Title  from Calendar_Category Where Site_Id = 1 AND CDN_Implementation = -1 " 
