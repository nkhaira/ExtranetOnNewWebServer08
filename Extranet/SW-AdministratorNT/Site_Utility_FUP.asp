...
''Get the system special folder
'set objSysfolder=fso.GetSpecialFolder(1)
'strMetbasePath=objSysfolder.path & "\inetsrv\Metabase.xml"
mapPath    = Server.mappath(request.Servervariables("SCRIPT_NAME"))
strScriptPath = mid(mapPath,1,instrrev(mapPath,"\",len(mapPath),vbTextCompare)-1)
mapPathLen = len(mapPath)
Dim fsoFolder
Dim strReadOnlyPath
'Response.Write Request.QueryString("Path")
'if fso.FileExists(strScriptPath & "\Metabase.xml")=true then
'    call fso.DeleteFile(strScriptPath & "\Metabase.xml",true)
'end if
'call fso.CopyFile(strMetbasePath,strScriptPath & "\Metabase.xml",true)
'set objstream = fso.OpenTextFile(strMetbasePath)
'strText=objstream.ReadAll
'Response.Write strtext
'response.Write strMetbasePath & " - " & strScriptPath
if Session("SWFUP_Path") = "" then
  Session("SWFUP_Path") = Server.mappath(request.Servervariables("SCRIPT_NAME"))
  strReadOnlyPath = Session("SWFUP_Path")
  strReadOnlyPath = mid(strReadOnlyPath,1,instrrev(strReadOnlyPath,"\",len(strReadOnlyPath),vbTextCompare)-1)
else
  strReadOnlyPath = Session("SWFUP_Path")
end if
...
do while(instrrev(cstr(strReadOnlyPath),"\",len(strReadOnlyPath),vbTextCompare) > 0)
    strReadOnlyPath=mid(strReadOnlyPath,1,instrrev(strReadOnlyPath,"\",len(strReadOnlyPath),vbTextCompare)-1)
    set fsoFolder=fso.GetFolder(strReadOnlyPath)
    if err.number=76 then
       FUP_Root = strReadOnlyPath & "\"
       exit do
    end if
    FUP_Root = strReadOnlyPath & "\"
loop
...
if request.querystring("getDRVs")="@" then
  ...
  with response
    ...
    .write "      <TABLE>" & vbCrLf
    .write "        <TR>" & vbCrLf
    .write "          <TD><TT><font color=white> Drive</TD>" & vbCrLf
    .write "          <TD><TT><font color=white> Type</TD>" & vbCrLf
    .write "          <TD><TT><font color=white> Path</TD>" & vbCrLf
    .write "          <TD><TT><font color=white> ShareName</TD>" & vbCrLf
    .write "          <TD><TT><font color=white> Size[MB]</TD>" & vbCrLf
    .write "          <TD><TT><font color=white> ReadyToUse</TD>" & vbCrLf
    ...
    for each thingy in fso.Drives
      ...
      .write "      <TR>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.DriveLetter & "</TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.DriveType   & "</TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.Path        & "</TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.ShareName   & "</TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & ((thingy.TotalSize)/1024000) & "</TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.IsReady     & " </TD>" & vbCrLf
      .write "        <TD><TT><font color=white> " & thingy.VolumeName
...
' --------------------------------------------------------------------------------------
Dim PreviousFolderArray,Foldercount
...
if trim(Request.QueryString("Path")) = "" then
    ...
    set folder = fso.GetFolder(Request.QueryString("Path"))
    if instr(MyPath,"..") > 0 then
        PreviousFolderArray=split(MyPath,"\")
        showPath=""
        for Foldercount=0 to ubound(PreviousFolderArray)-2
            showPath= showPath & PreviousFolderArray(Foldercount) & "\" 
        next
    else
        showPath = MyPath & "\"
    end if
...
select case request.form("cmd")
	...
	case "Delete"
      ...
      CommandToDo = "Deleting: " & request.Form("txtpath") & "\" & request.Form("Fname")
			response.flush
 			fso.DeleteFile request.Form("txtpath") & "\" & request.Form("Fname")
    else
 			CommandToDo = "Deleting: " & request.Form("txtpath") & "\???"
 			err.number = 424
 		end if
...
with response
  ...
  .write "sub getit(thestuff,path)" & vbCrLf
  .write "  if right(""" & showPath & """,1) <> ""\"" then" & vbCrLf
  .write "    document.myform.txtpath.value = """ & showPath & """ & ""\"" & thestuff" & vbCrLf
  .write "  else" & vbCrLf
  .write "    document.myform.txtpath.value = """ & showPath & """ & thestuff" & vbCrLf
  .write "  end if" & vbCrLf
  .Write "  document.myform.action=""Site_Utility_Fup.asp?Path="" & path" & vbCrLf
...
with response
  ...
  .write "    <TR>" & vbCrLf
  .write "      <TD BGCOLOR=#505050><FONT FACE=Arial SIZE=-2 COLOR=#99FFFF><div ></div></TD>" & vbCrLf
  .write "      <TD ALIGN=RIGHT><FONT FACE=Arial SIZE=-2 COLOR=#99FFFF>Volume Label:&nbsp;&nbsp;</FONT>&nbsp;</TD>" & vbCrLf
  .write "    </TR>" & vbCrLf
...
' --------------------------------------------------------------------------------------
Dim mapPath
mapPath=Request.QueryString("Path")
with response
  ...
  if Rootfolder=false then
    if Request.QueryString("Path")="" then
        ...
        PreviousFolderArray=split(showPath,"\")
        PreFolder=""
        for Foldercount=0 to ubound(PreviousFolderArray)-2
            PreFolder= PreFolder & PreviousFolderArray(Foldercount) & "\" 
        next
        PreFolder=left(PreFolder,len(PreFolder)-1)
        if fso.FolderExists(PreFolder)=true then
            .write "<FONT face=wingdings color=Gray >0</FONT> <FONT COLOR=#c8c8c8><span style='cursor: hand;' OnClick=""getit '..',''"">..</span></FONT><BR>" & vbCrLf
        else
            .write "<FONT face=wingdings color=Gray >0</FONT> <FONT COLOR=#c8c8c8><span style='cursor: hand;' OnClick=""getit '..','" & mid(mapPath,1,instrrev(mapPath,"\",-1)-1) & "'"">..</span></FONT><BR>" & vbCrLf    
        end if            
    end if	    
  end if	
  for each fold in folder.SubFolders
    fo = fo + 1
  	if Request.QueryString("Path")="" then
        ...
        .write "<FONT face=wingdings color=Gray >0</FONT> <FONT COLOR=#eeeeee><span style='cursor: hand;' OnClick=""getit '" & fold.name & "','" & fold.path & "'"">" & fold.name & "</span></FONT><BR>" & vbCrLf    
    end if  	    
  next  
  ...
  '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    Dim rootfolderpaths()
    Dim iFolders
    LastFolder=split(MyPath,"\")
    iRootSites=0
    For Each objItem in colItems 
        if ucase(right(objItem.Name,4)) = "ROOT" then
            redim preserve rootfolderpaths(2,iRootSites)
            rootfolderpaths(0,iRootSites) = objItem.Name
            rootfolderpaths(1,iRootSites) = objItem.Path
            iRootSites=iRootSites+1
        end if
    next
    Dim strDir
    Dim strDirPath
    ...
    For Each objItem in colItems 
        if Ucase(right(objItem.Name,4)) <> "ROOT" then
            for f=0 to ubound(rootfolderpaths,2)
                if instr(1,objItem.Name,rootfolderpaths(0,f)) > 0 then
                    strDir=replace(objItem.Name,rootfolderpaths(0,f),rootfolderpaths(1,f))
                    strDir=replace(strDir,"\","/")
                    if InStr(1,ucase(strDir),replace(ucase(MyPath),"\","/")) > 0 then
                        strDirPath = objItem.Path
                        dirFound=true
                        exit for
                    end if    
                end if
 ...
 Cont:   
    if dirFound=true then
        FolderPath=split(strDir,"/")
        For f=0 to ubound(FolderPath)
            if ucase(FolderPath(f))=ucase(LastFolder(ubound(LastFolder))) then
                'Response.Write FolderPath(f+1)
                .write "<FONT face=wingdings color=Gray >0</FONT> <FONT COLOR=#eeeeee><span style='cursor: hand;' OnClick=""getit '" & FolderPath(f+1) & "','" & strDirPath & "'"">" & FolderPath(f+1) & "</span></FONT><BR>" & vbCrLf
                exit for
            end if
        Next
    end if
