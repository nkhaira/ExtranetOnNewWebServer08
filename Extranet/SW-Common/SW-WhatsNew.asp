<%
  ...
  do while not rsCategory.EOF
   ...
   'RI#1874
   if (site_id="11") then 
     if Show_Days = 1 then Show_Days = 365
     if Show_Days = 6 then Show_Days = 180
   end if  
   ...
   if (Access_Level <= 2 or Access_Level = 6) then
    ...
    'RI#1874
    if (site_id="11") then 
     if Show_Days = 365 then Show_Days = 1
     if Show_Days = 180 then Show_Days = 6
    end if 

    
      'Response.Write "show_days" & Show_days
      'response.Write(SQL)
      'response.End()
...
'    response.write SQL & "<P>"
  ...
  if not WhatsNewFlag then
    response.write Translate("There have been no New Items Posted to this Site within the past",Login_Language, conn) & " " & Show_Days & " " 
    'RI#1874 
    if (site_id="11") then 
    if Show_days=6 then response.write Translate("months",Login_Language,conn) & "."
    if Show_days=1 then 
        response.write Translate("year",Login_Language,conn) & "." 
    else 
         if Show_Days>=7 then response.write Translate("days",Login_Language,conn) & "."
    end if
    else
...
sub WhatsNew_Filter()
  ...
  response.write ">7"&"&nbsp;"  
  if site_id = "11" then      response.write Translate("Days",Login_Language,conn)    
 ...
 if (site_id="11") then 
  ...
  if Show_Days = 6 then    response.write " SELECTED"
  response.write ">6"&"&nbsp;"  
  response.write Translate("Months",Login_Language,conn)
  response.write "</OPTION>" & vbCrLf
  
  response.write "        <OPTION VALUE=""1"""          
  if Show_Days = 1 then   response.write " SELECTED"
  response.write ">1"&"&nbsp;"  
  response.write Translate("Year",Login_Language,conn)
  response.write "</OPTION>" & vbCrLf
 end if
