...
strHttpReferer = Request.querystring("HTTP_REFERER")
...
' --------------------------------------------------------------------------------------
if instr(1,lcase(strHttpReferer),"invision.flukenetworks.com") <= 0 then
%>
...
<form name="<%=FormName%>" method="post">
    <p>
        <table cellpadding="0" align="CENTER" width="100%" border="0" id="ContentTable">
            ...
            <tr><td></td><td></td><td></td></tr>
            <tr>
                <td></td><td class="SmallBold"><%=Translate("Products",Login_Language,conn) %>
                </td><td align="left">
                    <% 
                         ...
                         if not(objcol is nothing) then
                             ...
                             strletters = ""
                             response.write "<SELECT Class=small style=""display:none;""  name=""PCat_LinkedProducts"" CLASS=Medium TITLE=""" & Translate("Select first alpha character of Product name.",Login_Language,conn) & """>" & vbCrLf
                             ...
                             for icol = 0 to objcol.childnodes.length-1
                                ...
                                response.write "<OPTION Class=Medium value=""" & objinfo1.text & """>" & mid(objinfo2.text,1,70) & "</OPTION>" & vbCrLf  
      	                     next 
                         ...
                         response.write "<select Class=small name=""PLCat_Category"" CLASS=Medium onchange=""ChangeProducts(this.options[this.selectedIndex].value,'PCat_LinkedProducts','PCat_LProducts')"" TITLE="""& Translate("Select first alpha character of Product name.",Login_Language,conn) & """>" & vbCrLf    
                         response.write "<option Value=""0"" SELECTED>" & Translate("Select",Login_Language,conn) & "</OPTION>" & vbCrLf
        
  		                 if blnnumbers = true then
				            if request("PLCat_Category") ="1" then
                              response.write "<OPTION Class=Medium Value=""1"" selected>0 - 9</OPTION>" & vbCrLf
				            else
					          response.write "<OPTION Class=Medium Value=""1"" >0 - 9</OPTION>" & vbCrLf
				            end if
  		                 end if

                         for icnt=1 to len(strfound)
				            if ucase(trim(request("PLCat_Category"))) =ucase(mid(trim(strfound),icnt,1)) then
                               response.write "<OPTION " & " selected " & " Class=Medium Value=""" & mid(strfound,icnt,1) & """ >" & mid(strfound,icnt,1) & "</OPTION>" & vbCrLf
				            else
                         ...
                         next
      	                 ...
      	                 response.write "</SELECT>" & vbCrLf
                    %>
                    <select name="PCat_LProducts" style="width=200" class="small" onchange="RetrieveAssets(this.options[this.selectedIndex].value);this.style.cursor='Wait'">
                    <%
                        ...
                        for icol=0 to objcol.childnodes.length-1
                           ...
                           if  ucase(mid(trim(objinfo2.text),1,1)) = ucase(trim(request("PLCat_Category")))   then
	                         if objinfo1.text = request("PCat_LProducts") then
					                response.write "<OPTION Class=Medium selected value=""" & objinfo1.text & """>" & mid(objinfo2.text,1,70) & "</OPTION>" & vbCrLf  
	                         else
					                response.write "<OPTION Class=Mediumvalue=""" & objinfo1.text & """>" & mid(objinfo2.text,1,70) & "</OPTION>" & vbCrLf  
	                         end if
            ...
            <tr>
                ...
                <td align="left">
                    <select name="PCat_Language" style="width=200" class="small" onchange="RetrieveAssets(this.options[this.selectedIndex].value);this.style.cursor='Wait'">
                        <%
                    Dim rs
            ...
            <tr>
                ...
                <td align="left">
                    <select name="PCat_Category" style="width=200" class="small" onchange="RetrieveAssets(this.options[this.selectedIndex].value);this.style.cursor='Wait'">
                    <%
            ...
            <tr>
                <td class="SmallBoldRed">
                    <div id="Standby" name="Standby" style="visibility: hidden">
                        <%=trim(Translate("Standby... Retrieving Assets for this Product",Login_Language,conn))%>
                    </div>
                </td>
                <td>
                    &nbsp;</td>
        ...
        <tr>
         ...
         <td bgcolor="#666666" align="left" width="45%" class="smallboldwhite">
       ...
       do while not rsCalendar.EOF and Record_Number <= Record_Limit
            ...
            <tr>
                <td bgcolor="#FFFFFF" align="left" class="small">
                  <%Response.Write "<a href=""http://www.flukenetworks.com/FNet/en-us/findit?Document=" & rsCalendar.Fields("item_number") & """>" & rsCalendar.Fields("Id") & "</a>"%> 
                </td>
                <td bgcolor="#FFFFFF" align="left"  class="small">
...
<!--
  ...
  {
    document.getElementById("Standby").style.visibility = "visible";
  	document.<%=FormName%>.action="SW-PCAT_FNET_ASSET_LIST.asp?Site_id=" + sid + "&Associate=" + relate;
