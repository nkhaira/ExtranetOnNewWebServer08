...
if len(request("Document")) = 5 or len(request("Document")) = 9 then  ' Information Store
  ...
  response.write "<TITLE>Stream It</TITLE>" & vbCrLf
  ...
  response.write "<FORM NAME=""FORM1"" ACTION=""/SW-Common/SW-Stream_It.asp"" METHOD=""POST"">" & vbCrLf
  ...
  elseif not isblank(request("Locator")) then
    LastSite = "Partner Portal Subscription URL Link: " & Server.URLEncode(Request.ServerVariables("SERVER_NAME") & "/Stream_It.asp?Locator=" & request("Locator"))
  elseif not isblank(request("SW-Locator")) then
    LastSite = "Partner Portal Asset URL Link: " & Server.URLEncode(Request.ServerVariables("SERVER_NAME") & "/Stream_It.asp?SW-Locator=" & request("SW-Locator"))
  else
