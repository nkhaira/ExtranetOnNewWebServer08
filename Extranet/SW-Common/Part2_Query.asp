...
Session("EquivRS").open SQL,DBConn,3,1,1

if limit > Session("EquivRS").recordcount then
	numpages = 1
else
	numpages = Session("EquivRS").recordcount \ limit
	numpages = cint(numpages)
	extrapage = Session("EquivRS").recordcount MOD limit
	if extrapage > 0 then
		numpages = numpages + 1
	end if
end if
...
Session("epart")       = strPart
Session("dept_id")     = dept_id

Call Disconnect_Sitewide
'Call Disconnect_Parts

response.redirect ("Part2_Results.asp?whatpage=1")
...
sub CheckValidity
    ...
    if isblank(tempPart) then
      if Session("LimitView") = CInt(false) then
    ...
    elseif ErrorString = "" and tempPartType = false then 'no errors so far for alpha key word
      ...
      if Session("LimitView") = CInt(true) then
        if not isnumeric(tempPart) then        
