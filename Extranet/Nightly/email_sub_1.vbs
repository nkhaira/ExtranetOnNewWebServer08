...
JMail.ServerAddress = "mail.evt.danahertm.com"
JMail.ServerPort = 25
	...
	for each Site_id in asset_sites
		...
		if not dbRS1.EOF then
			...
			if Site_iD = 10 or Site_iD = 1 then
				JMail.SenderName ="Fluke Calibration Partner Portal - Electrical - RF - Power/Energy"
			elseif Site_iD=5 then
				JMail.SenderName ="Fluke Calibration Partner Portal - Pressure - Flow"
			else
				JMail.SenderName = Trim(dbRS1("FromName"))
			end if 
