...
if ((isSubmit = 1 and len(strSelectedCat) > 0) OR (strRequestID <> "" OR strRequestID <> null)) then
    ...
    if (isSubmit = 1 and len(strSelectedCat) > 0) then
        ...
        ' SQL = SQL & " WHERE Site_ID = 3 AND DATEDIFF(day,Udate, getdate()) <= 2 "
        SQL = SQL & " WHERE Site_ID = 3 "
    ...
    elseif (strRequestID <> "" OR strRequestID <> null) then
        ...
        SQL = SQL & " FROM Calendar LEFT OUTER JOIN Language  "
        SQL = SQL & " ON Language.Code = Calendar.Language "
        SQL = SQL & " WHERE Site_ID = 3 and not (pid = -1 or pid = 0) "
        SQL = SQL & " AND (CDN_Required =0 OR CDN_Required is null )  "
    ...
    Do While not rsAssets.EOF 
      ...
      strFTPFilePath = ""
       strFTPFilePath = FTPFile(Server.MapPath("/FInd-Sales/" & rsAssets("File_Name")), rsAssets("File_Name")) ''FTP the file
...
function FTPFile(sSourceFile, sFTPLocation)
    ...
    Dim fsoRenameFluke_FileServerFile
    Set fsoRenameFluke_FileServerFile = CreateObject("Scripting.FileSystemObject")
    
    Dim FlukeFleNameBeforeModification : FlukeFleNameBeforeModification  =  fsoRenameFluke_FileServerFile.getfilename(sSourceFile)
    Dim FlukeFileNameAfterModification : FlukeFileNameAfterModification = "AmsToCDNUploaded" & FlukeFleNameBeforeModification
    
    Dim FlukeCdnUploadedFilePath : FlukeCdnUploadedFilePath = Replace(sSourceFile,FlukeFleNameBeforeModification, FlukeFileNameAfterModification)
    
    if fsoRenameFluke_FileServerFile.FileExists(sSourceFile) Then
        if fsoRenameFluke_FileServerFile.FileExists(FlukeCdnUploadedFilePath) then
            on error resume next
            fsoRenameFluke_FileServerFile.DeleteFile FlukeCdnUploadedFilePath, True ' Delete Record, True=Read Only and non-Read Only
            on error goto 0
        end if 
                              
        fsoRenameFluke_FileServerFile.MoveFile sSourceFile,FlukeCdnUploadedFilePath
     end if 

    if err.number <> 0 then 
       ' Response.Write "girish" & err.Description &vbctrlf
...
function ValidateFTPFileName(sFileNameToFTP, sID)
   ...
   ' sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=" & CInt(Site_ID) & " AND CDNFilePath = '" & sFileNameToFTP & "'"
    sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=3 AND CDNFilePath = '" & sFileNameToFTP & "'"
...
function UpdateCalendarFTPDetail(sID, sCDNRequried, sFTPLocation)
    Dim sSQL
    sSQL = "UPDATE Calendar SET CDN_Required = -1,CDNFilePath='" & sFTPLocation & "' WHERE Site_ID=3 AND Id = " & sID 
...
function DisplayCategories() 
	...
	sqlCategory = "SELECT ID, Title  from Calendar_Category Where Site_Id = 3 AND CDN_Implementation = -1 " 
