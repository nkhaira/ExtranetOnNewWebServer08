...
<DIV ALIGN=LEFT>
<%="<B>" & Translate("*Origin",Login_Language,conn) %>

</div>

<DIV ALIGN=LEFT>
<%="</BR><B>" & Translate("The Country of Origin provided by Fluke Corporation (“Fluke”) is intended to comply with World Customs Organization origin rules.  This information, however, may not be used for purpose of qualifying Fluke products for any Free Trade Agreement, the Trade Agreements Act, the Buy America Act, or any other government contract requirements.  If you require origin information for any of these purposes, please contact  ",Login_Language,conn)& vbCrLf%> 
<%="<A HREF=""mailto:countryoforigin@fluke.com""><B>" & Translate("  countryoforigin@fluke.com",Login_Language,conn) & "</B></A>."%>

</DIV>

<DIV ALIGN=LEFT>
<%="</BR><B>" & Translate("Sourcing and manufacturing processes change periodically.  Such changes may impact the Country of Origin.  You should periodically confirm Country of Origin with Fluke.",Login_Language,conn)%>

</div>

<%="</BR><B>" & Translate("Fluke is in no way responsible for any damages or penalties suffered by you as a result of using or relying upon such information.",Login_Language,conn) & "</BR>"%>
  ...
  %>
    <TABLE CELLPADDING=2 CELLSPACING=1 BORDER=0  WIDTH="100%">
      <TR>
        ...
        <TD BGCOLOR="#000000" ALIGN="CENTER" CLASS=SMALLBOLDGOLD><%Response.write Translate("ATP",Login_Language,conn)%><BR><%=Translate("Days",Login_Language,conn)%></TD>                                          
    ...
    if dept_id = 20 then
      ...
      ' ATP
        ...
        if not isnumeric(Session("rs")("atp")) then
          ...
          atp = Session("rs")("atp")  ' Convert to days
          if atp < 0 then atp = 0
          response.write atp
        end if  
   		else
