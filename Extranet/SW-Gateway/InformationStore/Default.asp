...
SQL =  "SELECT UserData.* FROM UserData WHERE UserData.Site_ID=" & Session("Site_ID") & " AND UserData.NTLogin='" & Session("Logon_User") & "'"
...
if not rsUser.EOF then		
  ...
  response.write "<FORM METHOD=""POST"" ACTION=""http://fluke.informationstore.net/extranet/default.asp"" ID=FORM1 NAME=FORM1>" & vbCrLf
  ...
  response.write "<INPUT TYPE=HIDDEN NAME=Groups                   VALUE=""" & rsUser("Groups") & """>" & vbCrLf
  response.write "<INPUT TYPE=HIDDEN NAME=SubGroups                VALUE=""" & rsUser("SubGroups") & """>" & vbCrLf

  response.write "<INPUT TYPE=HIDDEN NAME=BackURL                  VALUE=""" & Session("BACKURL") & """>" & vbCrLf

  response.write "<INPUT TYPE=HIDDEN NAME=Image_Locator            VALUE=""" & Request("Image_Locator") & """>" & vbCrLf
