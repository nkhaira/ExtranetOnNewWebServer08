...
sub Display_Category_Item
  ...
  if not isblank(Field_Data(xTitle)) then
    ...
    if CInt(Field_Flag(xSub_Category)) = CInt(True) and CInt(Title_View) = CInt(True) and (CIN < 8000 or CIN > 8999) then
  ...
  if CInt(Field_Flag(xBDATE)) = CInt(True) and CInt(Field_Flag(xEDate)) = CInt(True) then
    ...
    if CInt(Field_Flag(xLDate)) = CInt(True) and (Access_Level = 4 or Access_Level >= 8) then
      ...
      elseif CInt(Field_Flag(xUDATE)) = CInt(True) then
  ...
  if CInt(Field_Flag(xLocation)) = CInt(True) then
    response.write "<TR>" & vbCrLf            
  ...
  if Show_Detail < 0 _
    and ((CInt(Field_Flag(xThumbnail))  = CInt(True)  _
    or  CInt(Field_Flag(xDescription))  = CInt(True)  _
    or  CInt(Field_Flag(xInstructions)) = CInt(True)  _
    or  CInt(Field_Flag(xItem_Number))  = CInt(True)  _
    or (CInt(Field_Flag(xItem_Number))  = CInt(True) and CInt(Field_Flag(xItem_Number_Show)) = CInt(True) and CInt(Field_Data(xItem_Number_Show)) = CInt(True)) _
    or (CInt(Field_Flag(xItem_Number))  = CInt(False) and not isblank(rsCalendar("Lit_Item"))) _
    or  CInt(Field_Flag(xFile_Name))    = CInt(True)  _    
    or  CInt(Field_Flag(xConfidential)) = CInt(True)  _    
    or  CInt(Field_Flag(xPEData))       = CInt(True))) then
    ...
    if CInt(Field_Flag(xThumbnail)) = CInt(True) and CInt(Show_Thumbnail) = CInt(True) then
      if CInt(Field_Flag(xFile_Name)) = CInt(True) then
        select case UCase(Mid(Field_Data(xFile_Name),Instr(1,Field_Data(xFile_Name), ".")))
          ...
          case else
            ...
            if CInt(Field_Flag(xFile_Name_Size)) = CInt(True) then response.write " | Size: " & FormatNumber(CDbl(CDbl(Field_Data(xFile_Name_Size) / 1024)),0) & " KBytes"
    ...
    if CInt(Field_Flag(xTitle)) = CInt(True) and CInt(Field_Flag(xDescription)) = CInt(False) then
      Field_Data(xDescription) = Field_Data(xTitle)
      Field_Flag(xDescription) = CInt(True)
    end if

    if CInt(Field_Flag(xDescription)) = CInt(True) then
      if not isblank(KeySearch) then
    ...
    if CInt(Field_Flag(xInstructions)) = CInt(True) then
    ...
    if (CInt(Field_Flag(xItem_Number)) = CInt(True) and CInt(Field_Flag(xItem_Number_Show)) = CInt(True) and CInt(Field_Data(xItem_Number_Show)) = CInt(True)) then
    ...
    ' Confidential
    if CInt(Field_Flag(xConfidential)) = CInt(True) then
      if CInt(Field_Data(xConfidential)) = CInt(True) then
    ...
    if CInt(Field_Flag(xPEDate)) = CInt(True) then
    ...
    if CInt(Field_Flag(xCountry)) = CInt(True) and (Access_Level = 2 or Access_Level = 4 or Access_Level >= 8) then
...
sub RecordNavBar
  ...
  if CInt(Field_Flag(xLDate)) = CInt(True) then
  ...
  if CInt(Field_Flag(xLink)) = CInt(True) or CInt(Field_Flag(xFile_Name)) = CInt(True) then
  ...
  ' View Link
  if CInt(Field_Flag(xFile_Name)) = CInt(False) and CInt(Field_Flag(xLink)) = CInt(True) then
    if CInt(Field_Data(xLink_PopUp_Disabled)) = CInt(True) then     
  ...
  ' Download Title
  if CInt(Field_Flag(xFile_Name)) = CInt(True) or CInt(Field_Flag(xArchive_Name)) = CInt(True) then
    response.write "&nbsp;&nbsp;" & Translate("Download",Login_Language,conn) & ":&nbsp;&nbsp;"
  else
    response.write "&nbsp;"          
  end if
  ...
  if CInt(Field_Flag(xArchive_Name)) = CInt(True) then

    response.write "<A HREF=""javascript:void(0);"" TITLE=""Download ZIP File"
    if CInt(Field_Flag(xArchive_Size)) = CInt(True) then response.write " | Size: " & FormatNumber(CDbl(CDbl(Field_Data(xArchive_Size) / 1024)),0) & " KBytes"
  ...
  elseif CInt(Field_Flag(xFile_Name)) = CInt(True) then
  ...
  ' Send File as Email Attachment  if CInt(Field_Flag(xFile_Name)) = CInt(True) then
  response.write "<TD HEIGHT=11 VALIGN=MIDDLE WIDTH=""15%"" CLASS=NavRecordLight>"

    if (CInt(Field_Flag(xFile_Name)) = CInt(True) and instr(1,LCase(Field_Data(xFile_Name)),".exe") = 0) and (CInt(Field_Flag(xFile_Name)) = CInt(True) and instr(1,LCase(Field_Data(xFile_Name)),".iso") = 0) or (CInt(Field_Flag(xArchive_Name)) = CInt(True) and instr(1,LCase(Field_Data(xFile_Name)),".iso") = 0) then
    ...
    ' Zip Version
    if CInt(Field_Flag(xArchive_Name)) = CInt(True) and instr(1,LCase(Field_Data(xFile_Name)),".iso") = 0 then    
  ...
  if CInt(Field_Flag(xLink)) = CInt(True) then     
    ...
    if (CID = 9003 and CIN >= 8000 and CIN <= 8999 and CInt(Field_Data(xCampaign)) = 0) then
    ...
    elseif CID = 9001 or CID = 9003 or CID = 9004 then
      ...
      if (Field_Data(xCode) >= 8000 and Field_Data(xCode) <= 8999 and CInt(Field_Data(xCampaign)) = 0) then     
        response.write "&nbsp;&nbsp;" & Translate("More Info",Login_Language,conn) & ":&nbsp;&nbsp;"   
      ...
      elseif CInt(Field_Flag(xFile_Name)) = CInt(True) then
        ...
        if (SendIt_Show = true and instr(1,LCase(Field_Data(xFile_Name)),".exe") = 0) and (SendIt_Show = true and instr(1,LCase(Field_Data(xFile_Name)),".iso") = 0) or ((SendIt_Show = true and CInt(Field_Flag(xArchive_Name)) = CInt(True)) and instr(1,LCase(Field_Data(xFile_Name)),".iso") = 0) then
  ...
  if CInt(Shopping_Cart) = CInt(True) _
  ...
  if CInt(Field_Flag(xLanguage)) = CInt(True) then
  ...
  if CInt(Field_Flag(xStatus)) = CInt(True) then  
  ...
  if CInt(Field_Flag(xID)) = CInt(True) then  
    if (Access_Level = 4 or Access_Level >= 8) and CInt(Field_Data(xID)) > 0 then
    ...
    elseif CInt(Field_Data(xID)) > 0 then       
...
sub Update_Fields
  ...
  for i = 0 to Field_Max
    ...
    if not isblank(myField) then
      ...
      Field_Flag(i) = CInt(True)
    else
      Field_Data(i) = ""
      Field_Flag(i) = CInt(False)
    end if
  next
...
if CInt(Session("Cart_Active")) = CInt(True) and CInt(Shopping_Cart) = CInt(True) and instr(1,LCase(request.servervariables("SCRIPT_NAME")),"/sw-administrator") = 0 then
  %>
