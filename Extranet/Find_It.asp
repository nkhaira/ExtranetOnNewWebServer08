...
elseif LCase(request("Src")) = "fnet" and request("style") = "82" and len(request("Document")) > 0 then
    if CurrentTier() = "DEV" then
        response.redirect "http://flukenetworksdev.danahertm.com/vftf/" & request("Document")
    elseif CurrentTier() = "TEST" then
        response.redirect "http://flukenetworkstst.danahertm.com/vftf/" & request("Document")
    else
        response.redirect "http://www.flukenetworks.com/vftf/" & request("Document")
    end if    
else
  ...
  response.write "<FORM NAME=""FORM1"" ACTION=""/SW-Common/SW-Find_It.asp"" METHOD=""POST"">" & vbCrLf 
  ...
  response.write "<INPUT TYPE=""HIDDEN"" NAME=""PassKey"" VALUE=""" & Session.SessionID & """>" & vbCrLf
