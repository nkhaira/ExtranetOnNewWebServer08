...
if instr(1,lcase(request("Action")),"send password") > 0 then
  ...
  set senderName = "Fluke Calibration Partner Portal - Electrical - RF -  Power/Energy"
  msg.From = """" & senderName & """" & MailFromAddress
  msg.Sender = """" & senderName & """" & MailFromAddress
  
  elseif Site_ID=5 then
	  'Mailer.FromName       ="Fluke Calibration Partner Portal - Pressure - Flow"
    set senderName = "Fluke Calibration Partner Portal - Pressure - Flow"
    msg.From = """" & senderName & """" & MailFromAddress
    msg.Sender = """" & senderName & """" & MailFromAddress
  else
	  'Mailer.FromName       = MailFromName
    msg.From = """" & MailFromName & """" & MailFromAddress
    msg.Sender = """" & MailFromName & """" & MailFromAddress
  end if 
...
'  Mailer.FromName      = MailFromName
  ...
  if not isblank(msg.To) then
    msg.To = msg.To & ";" & """" & User_Name & """" & User_Email
  else
    msg.To = """" & User_Name & """" & User_Email
  end if
  ...
  if lcase(Account_ID) = "new" then
    ...
    if not rsApprovers.EOF then
      ...
      if CInt(rsApprovers("Email_Site_Admin")) = CInt(True) then
        ...
        if not isblank(msg.Bcc) then
          msg.Bcc = msg.Bcc & ";" & """" & MailFromName & """" & MailFromAddress
        else
          msg.Bcc = """" & MailFromName & """" & MailFromAddress
        end if
      end if
      ...
      if not rsUser.EOF then
        if Approver_Flag = False then
          ...
          msg.ReplyTo = rsUser("EMail")
          'Mailer.AddRecipient   rsUser("FirstName") & " " & rsUser("LastName"), rsUser("EMail")

          if not isblank(msg.To) then
            msg.To = msg.To & ";" & """" & rsUser("FirstName") & " " & rsUser("LastName") & """" & rsUser("EMail")
          else
          ...
          msg.Cc = """" & rsUser("FirstName") & " " & rsUser("LastName") & """" & rsUser("EMail")
        end if    
      end if
...
sub Send_EMail
  ...
  msg.To = msg.To & ";" & "santosh.tembhare@fluke.com"
  ...
  Else
    msg.From = """" & "Support Postmaster" & "extranetalerts@fluke.com"
    msg.To = """" & "Extranet Admin Group" & "extranetalerts@fluke.com"
    msg.Subject = "Send Email Failure"
    msg.TextBody = Err.description
    msg.Send 
