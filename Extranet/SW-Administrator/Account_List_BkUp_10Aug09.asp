      ...
      if SC2 <> 90 then
        ...
        Do while not rsUser.EOF and Record_Number <= Record_Limit
          ...
          select case SC5
			...
			''response.write "<br>------------"
				''dim strSQL, strCalib, strPress, strTemp
				''dim rsArr
				''set rsArr=Server.CreateObject("Adodb.recordset")
				''strSQL= "SELECT Site_Short_Descr from Site where Site_Code='Calibration-Sales'"
				''rsArr.open strSQL,conn, 3,3
				''if not rsArr.eof then
				''	strCalib=rsArr("Site_Short_Descr")
				''end if
				''rsArr.close
				''strSQL= "SELECT Site_Short_Descr from Site where Site_Code='DHI-Sales' or Site_Code='dhi-sales'"
				''rsArr.open strSQL,conn, 3,3
				''if not rsArr.eof then
				''	strPress=rsArr("Site_Short_Descr")
				''end if
				''rsArr.close
				''strSQL= "SELECT Site_Short_Descr from Site where Site_Code='Hart-Sales' or Site_Code='hart-sales'"
				''rsArr.open strSQL,conn, 3,3
				''if not rsArr.eof then
				''	strTemp=rsArr("Site_Short_Descr")
				''end if
				''rsArr.close

			''dim strSplit
			''strSplit=Trim(rsUser("Groups_Aux"))
			''strSplit=replace(rsUser("Groups_Aux"),"Calibration-Sales", strCalib)
			''strSplit=replace(strSplit,"DHI-Sales", strPress)
			''strSplit=replace(strSplit,"dhi-sales", strPress)
			''strSplit=replace(strSplit,"Hart-Sales", strTemp)
			''strSplit=replace(strSplit,"hart-sales", strTemp)
			''response.write strSplit


		    dim GroupsArr
		    dim GroupsSplit
		    dim iArr
		     dim strSQL, strWriteArr, strTest
			dim rsArr
			set rsArr=Server.CreateObject("Adodb.recordset")
		     GroupsArr=rsuser("Groups_Aux")
		     GroupsSplit=Split(GroupsArr,",")
			strWriteArr=""
			'strTest=""
				
			for iArr=0 to ubound(GroupsSplit)
				'response.write GroupsSplit(iArr)
				'strSQL=""
				strSQL= "SELECT Site_Short_Descr from Site where Site_Code='" & Trim(GroupsSplit(iArr)) & "'"
				
				rsArr.open strSQL,conn, 3, 3
				if not rsArr.eof then
						strWriteArr=rsArr("Site_Short_Descr")	
						if iArr=ubound(GroupsSplit) then
								response.write strWriteArr 									
						else
								response.write strWriteArr & ", "
						end if
				else
