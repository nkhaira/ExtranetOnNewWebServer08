...
'                  end if  
            ...
            <% if Show_Link = True or Show_Forum = True then %>
    	        ...
    	        <TD BGCOLOR="White" ALIGN=LEFT CLASS=Medium>
                <INPUT TYPE="Text" NAME="URLLink" SIZE="50" MAXLENGTH="255" VALUE="<%=RestoreQuote(sURLLink)%>" CLASS=Medium>
              </TD>
            </TR>
            <% else %>
...
<%end if%>
    ...
    %>
                ...
                do while not rsLanguage.EOF
                  ...
                  if not lcase(rs("Language")) = rsLanguage("Code") then
                    response.write " +"
                    end if
                  end if
            ...
            if (Show_PID = true) then
              if (PID_System = 0 And Site_ID <> 82) then
				...
				<% if CLng(rs("PID")) > 0 then %>
	                      <input type='button' CLASS='NavLeftHighlight1' onclick="Win=window.open('/sw-administrator/SW-PCAT_FNET_SAVE_ProductMultilingual.asp?ID=<%= rs("PID") %>&SiteId=<%= Site_ID %>','Locale_Window','status=no,height=550,width=450,scrollbars=yes,resizable=yes,toolbar=yes,links=no,left=300,top=300');Win.focus();return false;" value='Localization' />
				<% else %>
	                      <input type='button' CLASS='NavLeftHighlight1' onclick="alert('This asset is not associated to Product or Web Categories. The localized information can be entered only for an asset which has product or web category association.');return false;" value='Localization' />
				<% end if %>
                    </TD> 
                  </TR>
             <% end if
