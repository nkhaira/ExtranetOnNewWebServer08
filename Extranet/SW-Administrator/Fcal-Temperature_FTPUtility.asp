...
<table  width="700"  cellpadding="0">
        <tr>
            <td><b>File FTP Utility (FCal - Temperature) </b>
            </td>
        </tr>
...
if ((isSubmit = 1 and len(strSelectedCat) > 0) OR (strRequestID <> "" OR strRequestID <> null)) then
    ...
    if (isSubmit = 1 and len(strSelectedCat) > 0) then
        ...
        'SQL = SQL & " WHERE Site_ID = 4  AND DATEDIFF(day,Udate, getdate()) <= 2 "
        SQL = SQL & " WHERE Site_ID = 4  "
    ...
    Do While not rsAssets.EOF 
      ...
      strFTPFilePath = ""
       strFTPFilePath = FTPFile(Server.MapPath("/Hart-Sales/" & rsAssets("File_Name")), rsAssets("File_Name")) ''FTP the file
      ...
      'if isnull(rsAssets("CDNFilePath")) OR rsAssets("CDNFilePath") = "" then
           ...
           ''3.2 FTP the file
           ''if (ValidateFTPFileName("Hart-Sales/" & rsAssets("File_Name"),rsAssets("ID"))) Then
...
function FTPFile(sSourceFile, sFTPLocation)
    ...
    'File re-naming ------------------------------
    Dim fsoRenameFcalTemp_FileServerFile
    Set fsoRenameFcalTemp_FileServerFile = CreateObject("Scripting.FileSystemObject")
    
    Dim FcalTempFleNameBeforeModification : FcalTempFleNameBeforeModification  =  fsoRenameFcalTemp_FileServerFile.getfilename(sSourceFile)
    Dim FcalTempFileNameAfterModification : FcalTempFileNameAfterModification = "AmsToCDNUploaded" & FcalTempFleNameBeforeModification
    
    Dim FcalTempCdnUploadedFilePath : FcalTempCdnUploadedFilePath = Replace(sSourceFile,FcalTempFleNameBeforeModification, FcalTempFileNameAfterModification)
    
    if fsoRenameFcalTemp_FileServerFile.FileExists(sSourceFile) Then
        if fsoRenameFcalTemp_FileServerFile.FileExists(FcalTempCdnUploadedFilePath) then
            on error resume next
            fsoRenameFcalTemp_FileServerFile.DeleteFile FcalTempCdnUploadedFilePath, True ' Delete Record, True=Read Only and non-Read Only
            on error goto 0
        end if 
                              
        fsoRenameFcalTemp_FileServerFile.MoveFile sSourceFile,FcalTempCdnUploadedFilePath
     end if 
    '---------------------------------------------
...
function ValidateFTPFileName(sFileNameToFTP, sID)
   ...
   ' sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=" & CInt(Site_ID) & " AND CDNFilePath = '" & sFileNameToFTP & "'"
    sSQL = "SELECT CDNFilePath FROM Calendar WHERE Site_ID=4 AND CDNFilePath = '" & sFileNameToFTP & "'"
...
function UpdateCalendarFTPDetail(sID, sCDNRequried, sFTPLocation)
    Dim sSQL
    sSQL = "UPDATE Calendar SET CDN_Required = -1,CDNFilePath='" & sFTPLocation & "' WHERE Site_ID=4 AND Id = " & sID 
...
function DisplayCategories() 
	...
	sqlCategory = "SELECT ID, Title  from Calendar_Category Where Site_Id = 4 AND CDN_Implementation = -1 " 
