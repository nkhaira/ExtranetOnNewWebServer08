...
response.write keysearch
  ...
  SQL     = "SELECT Calendar.*, " &_
            ...
            " INNER JOIN Asset_Category assetCat ON Calendar.[ID] = assetCat.AssetId INNER JOIN Calendar_Category calCat ON assetCat.CategoryId = calCat.[ID] " & _
  ...
  next
  sqlTemp = "       convert(varchar(50),   COALESCE(calCat.Category, '')) + " &_
...
elseif CID = 9004 and CINN = 2 Then    ' Search Literature
  ...
  if(CInt(Site_ID) = 82) then
        ...
        SQL = "SELECT Calendar.*, " &_
            ...
            "LEFT OUTER JOIN dbo.Literature_Items_US ON dbo.Lit_Cost_Center.Cost_Center = dbo.Literature_Items_US.COST_CENTER " &_
            "LEFT OUTER JOIN dbo.Calendar " &_
            "INNER JOIN Asset_Category assetCat ON Calendar.[ID] = assetCat.AssetId INNER JOIN Calendar_Category calCat ON assetCat.CategoryId = calCat.[ID] ON dbo.Literature_Items_US.ITEM = dbo.Calendar.Item_Number " &_
            " WHERE  (Lit_Cost_Center.Site_ID = " & Site_ID & ") " &_
            "AND Literature_Items_US.Status = 'Active' " &_
            "AND (Literature_Items_US.Action = 'Complete' " &_
            "OR (Literature_Items_US.Action = 'Incomplete' AND Literature_Items_US.Revision <> 'A')) "
  ...
  if CInt(Shopping_Cart) = CInt(True) then
    ...
    if(CInt(Site_ID) = 82) then
        ...
        SQL = "SELECT Calendar.*, " &_
            ...
            "RIGHT OUTER JOIN " &_
            "      dbo.Calendar ON dbo.Literature_Items_US.REVISION = dbo.Calendar.Revision_Code AND " &_
            "      dbo.Literature_Items_US.ITEM = dbo.Calendar.Item_Number INNER JOIN Asset_Category assetCat ON Calendar.[ID] = assetCat.AssetId " &_
            "WHERE Calendar.Site_ID=" & CInt(Site_ID) &" AND assetCat.CategoryId = " & CInt(CINN)
    end if
  else
   ...
   if(CInt(Site_ID) = 82) then
        ...
        SQL = "SELECT Calendar.*, NULL as Lit_PSize, NULL AS Lit_Colors, Null as Lit_Code, Item_Number AS Lit_Description, Item_Number AS Lit_Item, " &_
        "(Select isnull(count(*),0) from Asset_accesscodes where assetid = calendar.id) as CheckCode FROM Calendar INNER JOIN Asset_Category assetCat ON Calendar.[ID] = assetCat.AssetId WHERE Calendar.Site_ID=" & CInt(Site_ID) &" AND assetCat.CategoryId = " & CInt(CINN)
   end if
  end if
