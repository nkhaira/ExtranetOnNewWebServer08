...
sub Main
  ...
  'SQLThumbs = "UPDATE dbo.Literature_Items_US SET PDF=-1 WHERE ([PRINT]=-1) OR (POD=- 1)"
  'dbConnSiteWide.execute SQLThumbs

  ' Now get records for analysis
  
  SQLThumbs = "SELECT dbo.Literature_Items_US.ITEM AS L_Item, dbo.Literature_Items_US.REVISION AS L_Revision, dbo.Literature_Items_US.STATUS AS L_Status, " &_
              "       dbo.Literature_Items_US.[ACTION] AS Action, dbo.Literature_Items_US.Inventory_Rule AS L_Inventory_Rule, dbo.Literature_Items_US.DESCRIPTION AS Description, " &_
              "       dbo.Literature_Items_US.MARCOM_ITEM_DESC AS Description_Marcom, dbo.Literature_Items_US.EFULFILLMENT AS Description_EDF, " &_
              "       dbo.Literature_Items_US.COST_CENTER AS Cost_Center, dbo.Literature_Items_US.[LANGUAGE] AS Language, " &_
              "       dbo.Literature_Items_US.[PRINT] AS Lit_Print, dbo.Literature_Items_US.PDF AS Lit_PDF, dbo.Literature_Items_US.WEB AS Lit_Web, " &_
              "       dbo.Literature_Items_US.CD AS Lit_CD, dbo.Literature_Items_US.DISPLAY AS Lit_Display, dbo.Literature_Items_US.DISKS AS Lit_Disks, " &_
              "       dbo.Literature_Items_US.VIDEO_NTSC AS Lit_Video_NTSC, dbo.Literature_Items_US.VIDEO_PAL AS Lit_Video_PAL, " &_
              "       dbo.Literature_Items_US.POD AS Lit_POD, dbo.Literature_Items_US.MARCOM_MANAGER AS L_Owner, dbo.Lit_Cost_Center.Site_ID AS CC_Site_ID, " &_
              "       dbo.Calendar.ID AS P_ID, dbo.Calendar.Item_Number AS P_Item, dbo.Calendar.Site_ID AS P_Site_ID," &_ 
              "       dbo.Calendar.Revision_Code AS P_Revision, dbo.Calendar.File_Name AS File_Name, dbo.Calendar.File_Name_POD AS File_Name_POD, " &_
              "       dbo.Calendar.Thumbnail AS Thumbnail, dbo.Calendar.Status AS P_Status, dbo.Calendar.Submitted_By AS P_Owner_ID, " &_
              "       dbo.Calendar.Campaign AS Campaign, dbo.UserData.LastName + ', ' + dbo.UserData.FirstName AS P_Owner, dbo.UserData.Email AS P_Email " &_
              "FROM   dbo.UserData INNER JOIN " &_
              "       dbo.Calendar ON dbo.UserData.ID = dbo.Calendar.Submitted_By RIGHT OUTER JOIN " &_
              "       dbo.Literature_Items_US LEFT OUTER JOIN " &_
              "       dbo.Lit_Cost_Center ON dbo.Literature_Items_US.COST_CENTER = dbo.Lit_Cost_Center.Cost_Center ON " &_
              "       dbo.Calendar.Item_Number = dbo.Literature_Items_US.ITEM " &_
              "WHERE      (dbo.Literature_Items_US.STATUS = 'Active' AND dbo.Literature_Items_US.[ACTION] = 'Complete' " &_
              "       OR  (dbo.Literature_Items_US.STATUS = 'Active' and dbo.Literature_Items_US.[ACTION] = 'incomplete' and (dbo.Literature_Items_US.Inventory_Rule = 'new_item' OR dbo.Literature_Items_US.Inventory_Rule = 'new_item') and dbo.Literature_Items_US.Revision <> 'a') " & _    
              "       OR  (dbo.Literature_Items_US.STATUS = 'Active' and dbo.Literature_Items_US.[ACTION] = 'incomplete' and (dbo.Literature_Items_US.Inventory_Rule = 'use_up' OR dbo.Literature_Items_US.Inventory_Rule = 'use_up')) " & _    
              "       OR  (dbo.Literature_Items_US.STATUS = 'Active' and dbo.Literature_Items_US.[ACTION] = 'incomplete' and (dbo.Literature_Items_US.Inventory_Rule = 'use_date' OR dbo.Literature_Items_US.Inventory_Rule = 'use_date')) " & _    
              "       OR  (dbo.Literature_Items_US.STATUS = 'Active' and dbo.Literature_Items_US.[ACTION] = 'incomplete' and dbo.Literature_Items_US.Inventory_Rule = 'reprint')) " & _    
              "       AND (dbo.Lit_Cost_Center.Site_ID <> 80 " &_
              "       AND  dbo.Lit_Cost_Center.Site_ID <> 19) " &_              
               "ORDER BY dbo.Literature.Cost_Center, dbo.Literature_Items_US.ITEM, dbo.calendar.ID"
 
'              "       AND (dbo.Literature_Items_US.Item=1260135) " &_
...
'response.end
            ...
            if CInt(rsThumbs("P_Status")) <= 1 then       ' In-Review or Live Only
              ...
              if rsThumbs("Lit_PDF")   = CInt(True) or _
                ...
                if rsThumbs("Lit_POD") = CInt(True) then  ' Search DB for POD File (Must Be Live)
                  ...
                  SQLPod = "SELECT dbo.Calendar.Subgroups, dbo.Calendar.File_Name_POD FROM dbo.Calendar WHERE dbo.Calendar.Item_Number='" & rsThumbs("L_Item") & "' AND dbo.Calendar.Status=" & CInt(True) & " AND dbo.Calendar.File_Name_POD IS NOT NULL"
                  if DevEnv then
                  ...
                  if not rsPOD.EOF then
                    ...
                    if instr(1,LCase(rsThumbs("Subgroups")),"view") > 0 then
              ...
              if not isblank(rsThumbs("P_Revision")) then
                if UCase(rsThumbs("L_Revision")) = UCase(rsThumbs("P_Revision")) then
  ...
  '        next
          ...
          if CInt(Asset_Missing) = CInt(True) then    
            ...
            strBody = strBody & "<FONT Style=""font-weight:Normal; font-size:8.5pt;font-family: Arial,Verdana;color:#000000; font-style: Normal;"">" &vbCrLf &_
                                ...
                                if CInt(Asset_Status(xRevision)) = CInt(True) then
                                  strBody = strBody & " <FONT COLOR=""RED""><B>Mismatch</B></FONT> Externet Asset Revision: <B>" & UCase(rsThumbs("P_Revision")) & "</B>" & vbCrLf
                                end if
                                ...
                                strBody = strBody &  "Owner: "    & rsThumbs("P_Owner") & "<BR>" & vbCrLf &_
                                                     "Asset ID: " & rsThumbs("P_ID") & "&nbsp;&nbsp;&nbsp;" & vbCrLf &_
                                                      "<A HREF=""http://Support.Fluke.com/Find_It.asp?Document=" & rsThumbs("P_Item") & """>" & vbCrLf &_
                                                      "View</A>&nbsp;&nbsp;&nbsp;" & vbCrLf &_
                                                      "<A HREF=""http://Support.Fluke.com/SW-Administrator/Calendar_Edit.asp?ID=" & rsThumbs("P_ID") & "&Site_ID=" & rsThumbs("P_Site_ID") & """>" &_
                                                      "Edit Asset</A><BR>" & vbCrLf &_
                                                      "Status: " & Status(rsThumbs("P_Status")) & vbCrLf &_
                                                      "<BR>" & vbCrLf
  
                                strBody = strBody & "Oracle Literature Media: "
          ...
          elseif CInt(Asset_Status(xItem_Number)) = CInt(True) then  
            ...
            strBody = strBody & "<FONT Style=""font-weight:Normal; font-size:8.5pt;font-family: Arial,Verdana;color:#000000; font-style: Normal;"">" &vbCrLf &_
                                "Item Number: <B>" & rsThumbs("L_Item") & "</B><BR>" & vbCrLf &_
                                "Revision: Oracle <B>" & rsThumbs("L_Revision") & "</B>" & vbCrLf
                                strBody = strBody & "<BR>" &_                                    
                                "Description: " & rsThumbs("Description") & "<BR>" & vbCrLf &_
                                "Description EDF: " & rsThumbs("Description_EDF") & "<BR>" & vbCrLf &_                              
...
sub Get_Mail_Body()
  ...
  strBody = "<HTML>" & vbCrLf & "<BODY>" & vbCrLf &_
            ...
            "<TR><TD><FONT STYLE=""font-size:8.5pt;font-family:Arial;"">PDF</FONT></TD><TD><FONT STYLE=""font-size:8.5pt;font-family: Arial;"">Low  - Resolution PDF File - Fulfillment through Support.Fluke.com</FONT></TD></TR>" &_                
            "<TR><TD><FONT STYLE=""font-size:8.5pt;font-family:Arial;"">POD</FONT></TD><TD><FONT STYLE=""font-size:8.5pt;font-family: Arial;"">High - Resolution PDF File - Fulfillment through Fluke Park Print Services (Requires PDF Version for Download)</FONT></TD></TR>" &_                
            "<TR><TD><FONT STYLE=""font-size:8.5pt;font-family:Arial;"">CD </FONT></TD><TD><FONT STYLE=""font-size:8.5pt;font-family: Arial;"">CD   - Fulfillment through DCG (Only Thumbnail Required)</FONT></TD></TR>" &_
...
sub Send_Mail
  ...
  Call Connect_Mailer
    ...
    msg.From = """Support.Fluke.com""" & "Webmaster@fluke.com"
    ...
    if not isblank(Asset_Owner) then
      ...
      for e = 0 to Asset_Owner_Email_Max
        ...
        if instr(1,Asset_Owner_Email(e),",") > 0 then
          ...
          SQLEmail = "SELECT  FirstName + ' ' + LastName AS Owner, Email AS Owner_Email " &_
                     "FROM    dbo.UserData " &_
                     "WHERE   (LastName = N'"  & Trim(Name(0)) & "') AND " &_
                     "        (FirstName = N'" & Trim(Name(1)) & "') AND " &_
                     "        (Site_ID=" & Site_ID & ") AND " &_
                     "        (SubGroups LIKE '%administrator%' OR SubGroups LIKE '%content%')"
          ...
          if not rsEmail.EOF then
            if not DevEnv then    
              ...
              Owner_Email_List = Owner_Email_List & rsEmail("Owner")
              Owner_Email_List = Owner_Email_List & " " & rsEmail("Owner_Email") & "<BR>"
            end if  
          end if
    ...
    if DevEnv then
      'strBody = Owner_Admin_List & Owner_Email_List & strBody
    end if  
    ...
    'Mailer.Subject = "Cost Center: " & Cost_Center & " " & "Extranet Missing Asset Report - " & Date()
    'Mailer.BodyText =  strBody
    msg.Subject = "Cost Center: " & Cost_Center & " " & "Extranet Missing Asset Report - " & Date()
