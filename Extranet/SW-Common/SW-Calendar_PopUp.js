...
//       ShowCalendar(document.[Form Name].[Click Image ID],document.[Form Name].[Field Name],null, '[Start Date]', '[End Date]')
function ShowCalendar(eP,eD,eDP,dmin,dmax,fnCB){
  ...
  if(null==wF.g_fCalLoaded || false==wF.g_fCalLoaded){
    alert("Unable to Load Calendar.\r\nPlease reload the page.\r\n\nIf you still are having difficulty, manually enter the date, then report this problem to your Site Administrator.");
    return;
  }
...
//    alert("Test Data: " + GetDOW2(1,7,2001) + " " + GetMonthCount(7,2001));  
}
...
function LoadMonths(n){
  ...
  for(i=0; i < n; i++){
    ...
    rg[i].src="/images/calendar/w" + GetDOW2(1,m,y) + "d" + GetMonthCount(m,y) + ".gif";
    m++;
    if(12 < m){
      m=1;
      y++;
