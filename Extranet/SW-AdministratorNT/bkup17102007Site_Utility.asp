...
if Admin_Access = 1 or Admin_Access = 2 or Admin_Access >= 3 then
  ...
  elseif (Utility_ID = 50 or Utility_ID = 51 or Utility_ID = 52 or Utility_ID=54 or Utility_ID = 60) and (Admin_Access = 2 or Admin_Access = 4 or Admin_Access >= 8) then
    ...
    response.write "<TD CLASS=Small>" & vbCrLf
    %>
    <SELECT CLASS=Small LANGUAGE="JavaScript" ONCHANGE="window.location.href='/SW-Administrator/site_utility.asp?ID=site_utility&Site_ID=<%=Site_ID%>&Utility_ID=<%=Utility_ID%>&Campaign=<%=Campaign%>&Submitted_By=<%=Submitted_By%>&FLanguage=<%=FLanguage%>&LDate=<%=LDate%>&Category_ID=<%=Category_ID%>&View=<%=View%>&Group_ID=<%=Group_ID%>&Sort_By=<%=Sort_By%>&Country='+this.options[this.selectedIndex].value;document.getElementById('Standby').style.visibility = 'visible';" NAME="Country">
    <%
    ...
    response.write "<TD CLASS=Small>" & vbCrLf
    %>
    <SELECT CLASS=Small LANGUAGE="JavaScript" ONCHANGE="window.location.href='/SW-Administrator/site_utility.asp?ID=site_utility&Site_ID=<%=Site_ID%>&Utility_ID=<%=Utility_ID%>&Campaign=<%=Campaign%>&LDate=<%=LDate%>&Category_ID=<%=Category_ID%>&View=<%=View%>&Country=<%=Country%>&Sort_By=<%=Sort_By%>&FLanguage=<%=FLanguage%>&Group_ID=<%=Group_ID%>&Submitted_By='+this.options[this.selectedIndex].value;document.getElementById('Standby').style.visibility = 'visible';" NAME="Submitted_By">
    <%
    ...
    SQL =       "SELECT distinct dbo.Calendar.*, dbo.Calendar_Category.Title AS Category, dbo.Calendar_Category.Sort, dbo.Literature_Items_US.STATUS AS Lit_Status, dbo.Literature_Items_US.STATUS_Name AS Lit_Status_Name, " &_
                ...
                "       dbo.Calendar.Status, dbo.Calendar.ID,(select top 1 language.sort from language where code=calendar.language) as languagesort " & _
                "FROM   dbo.Calendar " &_
                "       LEFT OUTER JOIN " &_
