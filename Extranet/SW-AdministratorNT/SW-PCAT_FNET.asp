...
''.write "<H1>tr1</H1>"
    ...
    if not IsNumeric(Calendar_ID) then
          ...
          'New record.
          response.write "<INPUT name=""PcatRelationNone"" onclick=""EnableDisableControls()"" TYPE = CHECKBOX VALUE=""-1"">"       
          showpcat = true
    else
            ...
            'dont allow to edit the checkbox again.
				...
				if PcatFlag=true then   
					if clng(rsvalidate.fields("PID").value) <= 0 then
						response.write "<INPUT onclick=""EnableDisableControls()"" disabled name=""PcatRelationNone"" TYPE = CHECKBOX checked VALUE=""" & rs("PID") &  """>" 	
					else
						response.write "<INPUT onclick=""EnableDisableControls()"" name=""PcatRelationNone"" TYPE = CHECKBOX checked VALUE=""" & rs("PID") &  """>" 
					end if
			    else				
				    response.write "<INPUT onclick=""EnableDisableControls()"" name=""PcatRelationNone"" TYPE = CHECKBOX checked VALUE=""" & rs("PID") &  """>" 
			    end if				
             ...
             response.write "<INPUT onclick=""EnableDisableControls()"" name=""PcatRelationNone"" title=""" & rs("PID") & """ TYPE = CHECKBOX VALUE=""" & rs("PID") &  """>" 
             showpcat = true
          end if 
...
''.write "<H1>tr4</H1>"
    ...
    if(Site_ID <> 82) then
        if (Request.QueryString("ID") <> "" and Request.QueryString("ID") <> "add") then
            .Write "<tr><TD>&nbsp;</TD></tr><tr><TD>Click here to enter multilingual data :<input type=button CLASS='NavLeftHighlight1' onclick=""Win=window.open('/sw-administratorNT/SW-PCAT_FNET_SAVE_ProductMultilingual.asp?ID=" & strPID & "&SiteId=" & Site_ID & "','Locale_Window','status=no,height=300,width=450,scrollbars=yes,resizable=yes,toolbar=yes,links=no,left=300,top=300');Win.focus();return false;"" value='Multilingual' /></TD></tr>"
        end if
    end if
...
''.write "<H1>tr5</H1>"
  ...
  if (Site_ID <> 82) then
    .write "<TR>" & vbCrLf
    .write "<TD BGCOLOR=""#EEEEEE"" VALIGN=TOP CLASS=Medium>" & vbCrLf
    .write Translate("Web Categories",Login_Language,conn) & ":"
    .write "</TD>" & vbCrLf
