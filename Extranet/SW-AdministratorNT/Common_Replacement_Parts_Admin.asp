...
if  Admin_Access = 3 or Admin_Access = 8 or Admin_Access = 9 then
...
' --------------------------------------------------------------------------------------  
  ...
  if sequence = "change model" then
    ...
    with response
      ...
      .write "<SPAN CLASS=SMALLBoldGold>" & Translate("Select Model",Login_Language,conn) & ": </SPAN>"
...
' --------------------------------------------------------------------------------------  
    ...
    .write "<SPAN CLASS=SMALLBoldGold>" & Translate("Model",Login_Language,conn) & ": </SPAN>"
    .write "<SELECT NAME=""Model"" CLASS=SMALL>" & vbCrLf
    ...
    .write "&nbsp;&nbsp;&nbsp;&nbsp;<SPAN CLASS=SmallBoldGold>" & Translate("Tools",Login_Language,conn) & ": </SPAN>" & vbCrLf
    ...
    if not isblank(BackURL) then
      ...
      .write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
      .write "<INPUT TYPE=BUTTON CLASS=NAVLEFTHIGHLIGHT1 NAME=BACKURL VALUE="" " & Translate("Exit",Login_Language,conn) & " "" LANGUAGE=""JavaScript"" ONCLICK=""window.close();"">" & vbCrLf
    end if
