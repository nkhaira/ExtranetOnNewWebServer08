...
' --------------------------------------------------------------------------------------
' Edit Content Item - Include File for /sw-administratorNT/Calendar_Edit.asp
'
    	  ...
    	  <TD WIDTH="100%" BGCOLOR="#EEEEEE">
            ...
            <!-- Header -->
              ...
              <TD BGCOLOR="Silver" VALIGN=TOP CLASS=Medium NOWRAP>                              
                ...
                <INPUT TYPE="Button" Title="Show/Hide Items in Site View Mode." onClick="location.href='/sw-administratorNT/Calendar_Edit.asp?ID=<%=Calendar_ID%>&Site_ID=<%=Site_ID%>&Show_View=<%if CInt(Show_View) = CInt(True) then response.write CInt(False) else response.write CInt(True)%>'" VALUE="<%if CInt(Show_View) = CInt(True) then response.write Translate("Hide Site View",Login_Language,conn) else response.write Translate("Site View",Login_Language,conn)%>" CLASS=Navlefthighlight1>
                <% end if %>
                <%
                
                if CInt(rs("Code")) >= 8000 AND CInt(rs("Code")) <= 8999 and Show_Content_Group then
                  response.write "&nbsp;&nbsp;&nbsp;&nbsp;"
                  response.write "<INPUT TYPE=""Button"" onClick=""location.href='/sw-administratorNT/Site_Utility.asp?ID=Site_Utility&Campaign=" & rs("ID") & "&Site_ID=" & Site_ID & "&Utility_ID=50&View=4'"" CLASS=Navlefthighlight1 Title=""List Individual Assets belonging to this Container."" VALUE=""" & Translate("List Assets",Login_Language,conn) & """>"
                end if
                ...
                if CLng(rs("Campaign")) > 0 then
                  ...
                  for i = Len(rs("Campaign")) + 1 to 5
                    response.write "0"
                  next
                  response.write rs("Campaign") & " "" CLASS=Navlefthighlight1 TITLE=""Go to PI/C Master Asset Container (MAC)"" onclick=""location.href='/sw-administratorNT/Calendar_Edit.asp?ID=" & rs("Campaign") & "&Site_ID=" & Site_ID & "'"">" & vbCrLf
                end if
...
'                      response.write "<A HREF=""javascript:void(0);"" onclick=""var MyPop1 = window.open('" & Edit_Content & "','MyPop1','fullscreen=no,toolbar=yes,status=no,menubar=no,scrollbars=yes,resizable=yes,directories=no,location=no,width=760,height=560,left=250,top=20'); MyPop1.focus(); return false;"" TITLE=""" & Translate("Click to Find this Asset on the Portal Site",Login_Language,conn) & """>" & rs("ID") & "</A>"    
                    ...
                    if Clone > 0 then
                      response.write "&nbsp;&nbsp;[ "
                      Edit_Content = "/sw-administratorNT/Calendar_Edit.asp?ID=" & Clone & "&Site_ID=" & Site_ID & "&Logon_User=" & Session("Logon_User")
                  ...
                  if Show_PID = true then
                    if  PID_System = 0 then 
                           if clng(rs.fields("clone").value) > 0 and clng(rs.fields("PID").value) > 0 then
                                response.write "<SELECT CLASS=Medium NAME=""Category_Change"" disabled LANGUAGE=""JavaScript"" ONCHANGE=""window.location.href='/sw-administratorNT/Calendar_Edit.asp?ID=" & Calendar_ID & "&Site_ID=" & Site_ID & "&Content_Group=" & Content_Group & "&Category_Change='+this.options[this.selectedIndex].value"" ONFOCUS=""Grouping_Name_Check();"">" & vbCrLf    
                           else
                                response.write "<SELECT CLASS=Medium NAME=""Category_Change"" LANGUAGE=""JavaScript"" ONCHANGE=""window.location.href='/sw-administratorNT/Calendar_Edit.asp?ID=" & Calendar_ID & "&Site_ID=" & Site_ID & "&Content_Group=" & Content_Group & "&Category_Change='+this.options[this.selectedIndex].value"" ONFOCUS=""Grouping_Name_Check();"">" & vbCrLf    
                           end if     
                    elseif PID_System = 1 then
                           response.write "<SELECT CLASS=Medium NAME=""Category_Change"" LANGUAGE=""JavaScript"" ONCHANGE=""window.location.href='/sw-administratorNT/Calendar_Edit.asp?ID=" & Calendar_ID & "&Site_ID=" & Site_ID & "&Content_Group=" & Content_Group & "&Category_Change='+this.options[this.selectedIndex].value"" ONFOCUS=""Grouping_Name_Check();"">" & vbCrLf    
                    end if
    ...
    %>                            
          ...
          <!-- Restricted to Countries -->
            ...
            if CInt(rs("Code")) >= 8000 AND CInt(rs("Code")) <= 8999 then
              if ((Clone = 0 and not rs("Locked") and (Admin_Access = 4 or Admin_Access >= 8)) or (Clone = 0 and Admin_Access >= 8)) and rs("Language") = "eng" then
                response.write "<INPUT TYPE=""BUTTON"" NAME=""Clone_All"" VALUE="" " & Translate("Clone MAC",Login_Language,conn) & " "" CLASS=Navlefthighlight1 ONCLICK=""window.location.href='/sw-administratorNT/Calendar_Admin_MAC_Clone.asp?Site_ID=" & Site_ID & "&MSID=" & Calendar_ID & "';"" TITLE=""Clone Master Asset Container and all associated Assets for language translation."">"
              else
                response.write "&nbsp;"
              end if
            else
...
'           if CInt(rs("Code")) >= 8000 AND CInt(rs("Code")) <= 8999 then
'           if ((Clone = 0 and not rs("Locked") and (Admin_Access = 4 or Admin_Access >= 8)) or (Clone = 0 and Admin_Access >= 8)) and rs("Language") = "eng" then
'           response.write "<INPUT TYPE=""BUTTON"" NAME=""Group_Update"" VALUE="" " & Translate("Update Groups",Login_Language,conn) & " "" CLASS=Navlefthighlight1 ONCLICK=""window.location.href='/sw-administratorNT/Calendar_Admin_Update_Groups.asp?Site_ID=" & Site_ID & "&MSID=" & Calendar_ID & "';"" TITLE=""Update Attached Asset Groups to Match MAC Container Groups."">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
'           end if
'           end if
