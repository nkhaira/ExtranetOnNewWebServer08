<%
Session("PopUpURL")=Request.ServerVariables("URL")&"?"& Request.ServerVariables("QUERY_STRING")
...
'Response.write "Content_Group is " & Content_Group 
'Response.write "<br> Show_Calendar is " & Show_Calendar 
'Response.write "<br> subgroups is " & rs("Subgroups")
'Response.write "content group " & Show_Content_Group
...
%>
<form name="<%=FormName%>" action="/SW-Administrator/Calendar_admin.asp?FileUpEE_Flag=<%=CInt(FileUpEE_Flag)%>&FileUpEE_Remote_Flag=<%=CInt(FileUpEE_Remote_Flag)%>"
    method="Post" enctype="MULTIPART/FORM-DATA" onkeyup="highlight(event)" onclick="highlight(event)"
    onsubmit="return CheckRequiredFields(this.form);">
    ...
    Call Table_Begin  
    %>
    <table width="100%" border="0" bordercolor="GRAY" cellpadding="0" cellspacing="0"
        align="CENTER">
        <tr>
                ...
                <table width="100%" cellpadding="4" border="0">
                    ...
                    <tr>
                        <td bgcolor="Silver" colspan="2" class="SmallBold">
                            <b>
                                <%=Translate("Note",Login_Language,conn)%>
                                :&nbsp;&nbsp;&nbsp;<img src="/images/required.gif" border="0" height="10" width="10">
                                =
                                <%=Translate("Required Information",Login_Language,conn)%>.
                        </td>
                        <td bgcolor="Silver" valign="TOP" class="Medium" nowrap>
                            <a href="#HELP" title="Click for Help Information on Asset Field Data Entry">
                                <img src="/images/help_button.gif" border="0" align="RIGHT" valign="TOP"></a>
                            <input type="Submit" name="Nav_Main_Menu" value=" <%=Translate("Main Menu",Login_Language,conn)%> "
                                class="Navlefthighlight1" title="Return to Administrators Main Menu." onclick="Menu_Button = true;">&nbsp;&nbsp;&nbsp;&nbsp;
                            <% if Show_Site_View = true then %>
                            <input type="Button" title="Show/Hide Items in Site View Mode." onclick="location.href='/SW-Administrator/Calendar_Edit.asp?ID=<%=Calendar_ID%>&Site_ID=<%=Site_ID%>&Show_View=<%if CInt(Show_View) = CInt(True) then response.write CInt(False) else response.write CInt(True)%>'"
                                value="<%if CInt(Show_View) = CInt(True) then response.write Translate("Hide Site View",Login_Language,conn) else response.write Translate("Site View",Login_Language,conn)%>"
                                class="Navlefthighlight1">
                            <% end if %>
                            <%
                  ...
                  response.write "&nbsp;"
                    ...
                    if Show_PID = true then
                          if  PID_System = 0 then  
                                   ...
                                   'if CInt(Site_ID) = 82 AND Show_CDN_Implementation = True then
                                   'RI#1705 -gpd
                                    'if (CInt(Site_ID) = 82 AND Show_CDN_Implementation = True) or  (CInt(Site_ID) = 3 AND Show_CDN_Implementation = True ) then
                                    if (Show_CDN_Implementation = True ) then
                                        response.write "<INPUT TYPE=""Hidden"" NAME=""Add_Edit_Flag"" VALUE=""Update"">"
                                        response.write "&nbsp;&nbsp;&nbsp;&nbsp;"
                                        response.write "<label id=""lblFTPMsg"" style=""color:red;""></label>"
                                   end if     
                                   '// Code End
...
'                      response.write "<A HREF=""javascript:void(0);"" onclick=""var MyPop1 = window.open('" & Edit_Content & "','MyPop1','fullscreen=no,toolbar=yes,status=no,menubar=no,scrollbars=yes,resizable=yes,directories=no,location=no,width=760,height=560,left=250,top=20'); MyPop1.focus(); return false;"" TITLE=""" & Translate("Click to Find this Asset on the Portal Site",Login_Language,conn) & """>" & rs("ID") & "</A>"    
                    ...
                    response.write "</TD>"
Dim assetStatus
assetStatus=rs("status")

'response.write "Admin access " & Admin_Access
'response.write "Show_Calendar is " & Show_Calendar 
'response.write "Field_Editable" & Field_Editable
'' RI-781
response.write "<input type=""hidden"" name=""assetStatus"" value=""" & assetStatus & """>"
                    ' Status Review
        ...
        <%
                ...
                if not rsDebug.EOF then
                  ...
                  do while not rsDebug.EOF
                    ...
                    select case UCase(rsDebug("Status_Name"))
                      case "COMPLETE", "FINAL LOADED", "WEB ONLY", "REPRINT"
                        response.write """#00FF00"">"
                      case else
                        response.write """#FFFF00"">"
                    end select
          ...
          if((Not IsNull(rs("Item_Number"))) And (rs("Item_Number") <> "")) then
            ...
            if not rsAllItems.EOF then
                if(isnumeric(rsAllItems("Counts")) and CInt(rsAllItems("Counts")) > 1) then
	                ...
	                if not rsDuplicateItems.EOF then
	                    response.write "<tr><td colspan=2>Duplicate Item Numbers/Status Information</td><td>"
	                    Call Nav_Border_Begin
	                    response.write "<TABLE BORDER=0 CELLSPACING=2 CELLPADDING=2 BGCOLOR=""WHITE"">" & vbCrLf
                        response.write "<TR>" & vbCrLf
                        response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & Translate("Item Number",Login_Language,conn) & "</TD>" & vbCrLf
                        response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & Translate("ID",Login_Language,conn) & "</TD>" & vbCrLf
                        response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & Translate("Rev No",Login_Language,conn) & "</TD>" & vbCrLf
                        response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & Translate("Status",Login_Language,conn) & "</TD>" & vbCrLf
                        response.write "</TR>" & vbCrLf
		                rsDuplicateItems.MoveFirst
		                
		                do while not rsDuplicateItems.EOF
		                
		                    response.write "<TR>" & vbCrLf
                            response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & rsDuplicateItems("Item_Number") & "</TD>" & vbCrLf
                            response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & rsDuplicateItems("ID") & "</TD>" & vbCrLf
                            response.write "<TD CLASS=SmallBold ALIGN=CENTER>" & rsDuplicateItems("Revision_Code") & "</TD>" & vbCrLf
                            if(rsDuplicateItems("Status") = 1) then
                                response.write "<TD CLASS=SmallBold ALIGN=CENTER>Live</TD>" & vbCrLf
                            elseif(rsDuplicateItems("Status") = 0) then
                                response.write "<TD CLASS=SmallBold ALIGN=CENTER>Review</TD>" & vbCrLf
                            else
                                response.write "<TD CLASS=SmallBold ALIGN=CENTER>Archive</TD>" & vbCrLf
                            end if
                            response.write "</TR>" & vbCrLf
		                rsDuplicateItems.MoveNext 
                        Loop
...
'                  end if  
        ...
        <% end if %>
        <!--Added on 6th Jan 2010, to show Govenment Compliance field for Software Products -->
        <% if CInt(Category_Code) = 9 AND CInt(Site_ID) = 82 then %>
        <tr>
            <td bgcolor="#EEEEEE" valign="TOP" class="Medium">
                <%
                    response.write Translate("Requires Government Compliance Check (encrypted software)",Login_Language,conn) & ":"
                %>
            </td>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <% 
                    response.write "<SELECT NAME=""GovtCompliance"" CLASS=Medium"">"
                    ...
                    if rs("GovernmentCompliance") = -1 then
                        response.write " SELECTED"
                    end if  
                    response.write ">" & Translate("Yes",Login_Language,conn) & "</OPTION>" & vbCrLf
                    
                    response.write "<OPTION CLASS=Medium VALUE=""0"" " 
                    if rs("GovernmentCompliance") = 0 OR isnull(request("GovernmentCompliance")) then
                        response.write " SELECTED"
                    end if  
                    response.write ">" & Translate("No",Login_Language,conn) & "</OPTION>" & vbCrLf
        ...
        <!--End -->
        <tr>
            <td colspan="3" bgcolor="Gray" class="Medium">
            </td>
        </tr>
        <!-- Item Number -->
        <%
            if Show_Item_Number = True then
              ...
              if isblank(rs("Item_Number")) then
                ...
                SQLin = "SELECT MAX(Item_Number) + 1 AS Next_Number " &_
                        "FROM dbo.Calendar " &_
                        "WHERE (Item_Number >= 9" & Site_ID_Pad & "0000) AND (Item_Number <= 9" & Site_ID_Pad & "9999) and site_id ="& site_ID &""                      
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <input type="Text" name="Item_Number" size="50" maxlength="20" value="<%=RestoreQuote(rs("Item_Number"))%>"
                    class="Medium" language="JavaScript" onchange="ck_item_number();">&nbsp;&nbsp;<%=Translate("Rev",Login_Language,conn)%>:&nbsp;<input
                        type="Text" name="Revision_Code" size="1" maxlength="4" value="<%=RestoreQuote(rs("Revision_Code"))%>"
                        class="Medium">&nbsp;
        ...
        <tr>
            <td bgcolor="#EEEEEE" valign="TOP" class="Medium">
                <%=Translate("Item / Reference Number",Login_Language,conn)%>
                &nbsp;2:&nbsp;<span class="Small">(<%=Translate("Legacy",Login_Language,conn)%>)</span>
            </td>
            <td bgcolor="#EEEEEE" align="CENTER" class="Medium">
                &nbsp;
            </td>
            <td bgcolor="White" align="LEFT" class="Medium">
                <input type="Text" name="Item_Number_2" size="50" maxlength="20" value="<%=RestoreQuote(rs("Item_Number_2"))%>"
                    class="Medium">
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td bgcolor="green" align="LEFT" class="Medium">
                <input type="Text" name="Product" size="50" maxlength="255" value="<%=RestoreQuote(rs("PRODUCT"))%>"
                    class="Medium" onfocus="Grouping_Name_Check();">
                <br>
                <select name="Product_New" class="Medium" onfocus="Grouping_Name_Check();">
                    <option class="Medium" value="">
                        <%=Translate("Enter Above or Select from this List",Login_Language,conn)%>
                    </option>
                    <%
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                if Show_PID = true then
                       if  PID_System = 0 then  %>
                <input type="Text" name="Title" size="50" maxlength="128" value="<%=RestoreQuote(rs("TITLE"))%>"
                    class="Medium" onfocus="Grouping_Name_Check();">
                <% elseif PID_System = 1 then  %>
                <input type="Text" name="Title" size="50" maxlength="255" value="<%=RestoreQuote(rs("TITLE"))%>"
                    class="Medium" onfocus="Grouping_Name_Check();">
                <% end if
                else%>
                <input type="Text" name="Title" size="50" maxlength="255" value="<%=RestoreQuote(rs("TITLE"))%>"
        ...
        <tr>
            <td bgcolor="#EEEEEE" class="Medium">
                <%=Translate("Location",Login_Language,conn)%>
                <span class="Small">(<%=Translate("City",Login_Language,conn)%>,
                    <%=Translate("State",Login_Language,conn)%>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                <input type="Text" name="Location" size="50" maxlength="255" value="<%=RestoreQuote(rs("LOCATION"))%>"
                    class="Medium">
            </td>
        </tr>
        <% else %>
        ...
        <!-- Link -->
        <% 
             if (IsNull(rs("LINK")) Or IsEmpty(rs("LINK"))) then
               sURLLink = ""
             else
               sURLLink = Replace(rs("LINK"), """", "'")
             end if %>
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <input type="Text" name="URLLink" size="50" maxlength="255" value="<%=RestoreQuote(sURLLink)%>"
                    class="Medium">
            </td>
        </tr>
        <% else %>
        <input type="HIDDEN" name="URLLink" value="<%=RestoreQuote(sURLLink)%>">
        <% end if %>
        ...
        <tr>
            <td bgcolor="#EEEEEE" class="Medium">
                <%=Translate("Content File",Login_Language,conn)%>
                <span class="Small">(HTM or ASP)</span>:
            </td>
            <td bgcolor="#EEEEEE" align="CENTER" class="Medium">
                &nbsp;
            </td>
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                %>
                <a href="http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("Include")%>" target=""
                    onclick="openit_mini('http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("Include")%>','Vertical');return false;">
                    <span class="NavLeftHighlight1">&nbsp;&nbsp;<%=Translate("View",Login_Language,conn)%>&nbsp;&nbsp;</span></a>&nbsp;&nbsp;&nbsp;
                <%
        ...
        %>
        <!--Added on 6th Aug 2010, to enable CDN Only if file upload is enabled-->
        
        <% if Show_CDN_Implementation = True AND Show_File = True then %>
        <tr>
            <td bgcolor="#EEEEEE" class="Medium" valign="TOP">
                <% response.write Translate("Accelerate using CDN",Login_Language,conn)& ": " %>
            </td>
            <td bgcolor="#EEEEEE" align="CENTER" class="Medium">
                &nbsp;</td>
            <td bgcolor="White" align="LEFT" class="Medium">
                <%
                        dim strCDNHiddVal
                        strCDNHiddVal = "False"
                        response.write "<INPUT TYPE=""Checkbox"" ID=""CDN_Required"" NAME=""CDN_Required"""
                        
                        if Show_CDN_DefaultState = "CheckedDisabled" Then   ''Forcefully Checked and Disabled
                            response.write " CHECKED DISABLED"
                            strCDNHiddVal = "True"
                        elseif Show_CDN_DefaultState = "CheckedEnabled" then  ''if null then by default Checked and Enabled, otherwise set value from database
                            if not isnull(rs("CDN_Required")) then
                                if rs("CDN_Required") = -1 then
                                   response.write " CHECKED"
                                   strCDNHiddVal = "True"
                                end if
                            else
                        ...
                        elseif Show_CDN_DefaultState = "UncheckedEnabled" Then     ''By default uncheked
                            if not isnull(rs("CDN_Required")) then
                                if rs("CDN_Required") = -1 then
                                   response.write " CHECKED"
                                   strCDNHiddVal = "True"
                        ...
                        response.write " onclick=""javascript:setCDN_RequiredInHidd(this)"" >"
                        
                        response.write "<INPUT TYPE=""Hidden"" ID=""hidCDN_Required"" NAME=""hidCDN_Required"" VALUE=""" 
                        response.write strCDNHiddVal
                        response.write """>"
                        
                        response.write "<INPUT TYPE=""Hidden"" NAME=""hidCDNFilePath"" VALUE=""" 
                        if not isnull(rs("CDNFilePath")) then
                             response.write rs("CDNFilePath") 
                        end if 
                        response.write """>"
                %>
            </td>
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                if isblank(rs("File_Name")) then
                  ...
                  'response.Write("CDN_Required"+strCDNHiddVal)
                  'response.End()
                %>
                
                <!--#include virtual="/SW-Administrator/Calendar_View.asp"-->
                <% 
                Dim FileLocation
                FileLocation = FTPFile
                'response.Write("Server value"+ FileLocation)
                'response.End()
                %>
                
               <% if(  (CInt(Site_ID)=46) and rs("CDN_Required") = -1  or (CInt(Site_ID)=1) and rs("CDN_Required") = -1 or (CInt(Site_ID)=4) and rs("CDN_Required") = -1 or (CInt(Site_ID)=5)) and rs("CDN_Required") = -1 or (CInt(Site_ID)=3)  and rs("CDN_Required") = -1 or (CInt(Site_ID)=29)  and rs("CDN_Required") = -1 then %>
               
                 <a href="http://<%=FileLocation%>/<%=rs("File_Name")%>"
                    target="" onclick="openit_mini('http://<%=FileLocation%>/<%=rs("File_Name")%>','Vertical');return false;">
                    <span class="NavLeftHighlight1">&nbsp;&nbsp;<%=Translate("View",Login_Language,conn)%>&nbsp;&nbsp;</span></a>&nbsp;&nbsp;&nbsp;
                                       
                <% else %>
                
                <a href="http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("File_Name")%>"
                    target="" onclick="openit_mini('http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("File_Name")%>','Vertical');return false;">
                    <span class="NavLeftHighlight1">&nbsp;&nbsp;<%=Translate("View",Login_Language,conn)%>&nbsp;&nbsp;</span></a>&nbsp;&nbsp;&nbsp;
                ...
                <%
                 ...
                 ' response.Write("Servervalue"+FileLocation)
                 ' response.End()
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                %>
                <a href="http://<%=Request("SERVER_NAME")%>/<%=rs("File_Name_POD")%>" target="" onclick="openit_mini('http://<%=Request("SERVER_NAME")%>/<%=rs("File_Name_POD")%>','Vertical');return false;">
                    <span class="NavLeftHighlight1">&nbsp;&nbsp;<%=Translate("View",Login_Language,conn)%>&nbsp;&nbsp;</span></a>&nbsp;&nbsp;&nbsp;
                <%
        ...
        <tr>
            <td bgcolor="#EEEEEE" class="Medium" valign="Top">
                ...
                : <span class="Small">- (GIF or JPG): </span>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                %>
                <a href="http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("Thumbnail")%>"
                    target="" onclick="openit_mini('http://<%=Request("SERVER_NAME")%>/<%=Site_Code%>/<%=rs("Thumbnail")%>','Vertical');return false;">
                    <span class="NavLeftHighlight1">&nbsp;&nbsp;<%=Translate("View",Login_Language,conn)%>&nbsp;&nbsp;</span></a>&nbsp;&nbsp;&nbsp;
                <%
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <input type="Text" name="Forum_ID" size="50" maxlength="10" value="<%=RestoreQuote(rs("Forum_ID"))%>"
                    class="Medium">
            </td>
        </tr>
        <!-- Forum Moderated -->
        ...
        <tr>
            <td bgcolor="#EEEEEE" class="Medium">
                <%=Translate("Beginning Date",Login_Language,conn)%>
            ...
            <td bgcolor="White" align="LEFT" valign="TOP" class="Medium">
                <input <%=field_editable%> type="Text" name="BDate" size="30" maxlength="10" value="<%=FormatDate(1, rs("BDATE"))%>"
                    class="Medium">&nbsp;&nbsp;
                <%
                if isblank(trim(Field_Editable)) then
                %>
                <a href="javascript:void()" language="JavaScript" onclick="window.dateField = document.<%=FormName%>.BDate;calendar = window.open('/sw-common/sw-calendar_picker.asp','cal','WIDTH=200,HEIGHT=250');return false">
        ...
        <tr>
            <td bgcolor="#EEEEEE" class="Medium">
                <%=Translate("Ending Date",Login_Language,conn)%>
                <span class="Small">(mm/dd/yyyy)</span>:
            </td>
            <td bgcolor="#EEEEEE" align="CENTER" class="Medium">
                <img src="/images/required.gif" border="0" width="10" height="10">
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <input <%=field_editable%> type="Text" name="EDate" size="30" maxlength="10" value="<%=FormatDate(1, rs("EDATE"))%>"
                    class="Medium">&nbsp;&nbsp;
                <%
                if isblank(trim(Field_Editable)) then
                %>
                <a href="javascript:void()" language="JavaScript" onclick="window.dateField = document.<%=FormName%>.EDate;calendar = window.open('/sw-common/sw-calendar_picker.asp','cal','WIDTH=200,HEIGHT=250');return false">
                    <img src="/images/calendar/calendar_icon.gif" border="0" height="21" align="TOP"></a>&nbsp;&nbsp;
                <%
                end if
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <input type="Text" name="XDays" size="30" maxlength="3" value="<%=rs("XDays")%>"
                    class="Medium">&nbsp;&nbsp<%=Translate("days after ending date",Login_Language,conn)%>
            </td>
        </tr>
        <% else %>
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                ...
                if isblank(trim(Field_Editable)) then
                %>
                <a href="javascript:void()" language="JavaScript" onclick="window.dateField = document.<%=FormName%>.PEDate;calendar = window.open('/sw-common/sw-calendar_picker.asp','cal','WIDTH=200,HEIGHT=250');return false">
                    <img src="/images/calendar/calendar_icon.gif" border="0" height="21" align="TOP"></a>&nbsp;&nbsp;
                <%
                end if
                %>
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <%
                       ...
                       response.write "<INPUT TYPE=""textbox"" size = ""100"" MAXLENGTH = ""1000"" NAME=""txtAccessCode"" value =""" & strPricelist & """ CLASS=Medium>"
    ...
    .write Translate("Partner Portal Categories",Login_Language,conn) & ":<br /><br />"
    .Write Translate("Hint: Select which partner portal categories that you want this asset to be related to.",Login_Language,conn)
    .write "</TD>" & vbCrLf

    'Required Icon or Space
    .write "<TD BGCOLOR=""#EEEEEE"" ALIGN=TOP>" 
    ...
    strSQL123 = "SELECT ID, Code, Title FROM dbo.Calendar_Category WHERE (Site_ID = " & CInt(Site_ID) & ") AND (Code <> 9999) AND Enabled=" & CInt(True) & " ORDER BY Title"
    ...
    .write "    <TD CLASS=Medium>" & vbCrLf
        ...
        <% 
    if showpcat=true then
       .write " <SELECT LANGUAGE=""JavaScript"" multiple size=""5"" ID=""PCat_SPortalCats"" NAME=""PCat_SPortalCats"" CLASS=""Medium"">"
	   else
       .write " <SELECT LANGUAGE=""JavaScript"" multiple size=""5"" ID=""PCat_SPortalCats"" NAME=""PCat_SPortalCats"" CLASS=""Medium"">"
    end if
    ...
    .write "</TR>" & vbCrLf            
            ...
            <td bgcolor="White" align="LEFT" class="MEDIUM">
                ...
                do while not rsLanguage.EOF
                  ...
                  elseif  rsLanguage("Code")="eng" then
                 	  response.write "<OPTION CLASS=Medium SELECTED VALUE=""" & rsLanguage("Code") & """>" & "English (Non Europe)"
                  ...
                  if not rsLanguage("Code")="eng" then
                    response.write " +"
                    end if
                  end if
        ...
        <%
            if (Show_PID = true) then
              if (PID_System = 0 And Site_ID <> 82) then
                if (Request.QueryString("ID") <> "" and Request.QueryString("ID") <> "add") then
        %>
        <tr>
            <td bgcolor="#EEEEEE" valign="TOP" class="Medium" colspan="2">
                <% response.write Translate("Multilingual data",Login_Language,conn) %>
                :
            </td>
            <td bgcolor="#EEEEEE" align="left" class="Medium">
                <% if CLng(rs("PID")) > 0 then %>
                                <!--<input type='button' class='NavLeftHighlight1' onclick="Win=window.open('/sw-administrator/SW-PCAT_FNET_SAVE_ProductMultilingual.asp?ID=<%= rs("PID") %>&SiteId=<%= Site_ID %>','Locale_Window','status=no,height=550,width=450,scrollbars=yes,resizable=yes,toolbar=yes,links=no,left=300,top=300');Win.focus();return false;"
                    value='Localization' />-->                    
                    
                   <input type='button' CLASS='NavLeftHighlight1'  onclick="openLocalizeWindow('<%= rs("PID") %>&SiteId=<%= Site_ID %>')" value='Localization' />
                <% else %>
                <input type='button' class='NavLeftHighlight1' onclick="alert('This asset is not associated to Product or Web Categories. The localized information can be entered only for an asset which has product or web category association.');return false;"
                    value='Localization' />
                <% end if %>
            </td>
        </tr>
        ...
        <!-- Added on 29th Oct 2009, for Business Unit and Ad Pixel -->
                ...
                <select name="Business_Unit" class="Medium">
                    <% 
                        ...
                        Do while not rsBusinessUnits.EOF
                          if lcase(rs("BusinessUnitCode")) = lcase(rsBusinessUnits("BusinessUnitCode")) then
                            response.write "<OPTION CLASS=Medium SELECTED VALUE=""" & rsBusinessUnits("BusinessUnitCode") & """>" & Translate(rsBusinessUnits("BusinessUnitDesc"),Login_Language,conn)
                          else
        ...
        <tr>
            ...
            <td bgcolor="White" align="LEFT" class="Medium">
                <%
                      response.write "<INPUT TYPE=""Hidden"" NAME=""txtAdPixel_B64"" VALUE="""">"
                      if (rs("AdPixel") <> "" ) then
                         response.write "<INPUT TYPE=""textbox"" size = ""75"" MAXLENGTH = ""500"" NAME=""txtAdPixel"" value =""" & Server.HTMLEncode(rs("AdPixel")) & """ CLASS=Medium>"
                      else
                        response.write "<INPUT TYPE=""textbox"" size = ""75"" MAXLENGTH = ""500"" NAME=""txtAdPixel"" value =""" & rs("AdPixel") & """  CLASS=Medium>"
                      end if
                %>
            </td>
        </tr>
                        ...
                        if Site_Id = 82 then
                              ...
                              elseif rsSubGroups("Enabled") = True then
                               ...
                               if Show_Marketing_Automation = True and rsSubGroups("Code") = "nfre" then
                                    ...
                                    if rs("FormProcessingURL") <> null or rs("FormProcessingURL") <> "" then
                                        if (instr(1,lcase(rs("SubGroups")),"nfre") > 0)  then
                                           response.write " CHECKED "         
                                        end if
                                    end if
                                    ...
                                    response.write "&nbsp;&nbsp;&nbsp;&nbsp; URL:&nbsp;&nbsp;"
                                    response.write "<INPUT TYPE=""textbox"" size = ""50"" MAXLENGTH = ""500"" NAME=""txtFormProcessingURL"" VALUE=""" & rs("FormProcessingURL") & """ CLASS=Medium></TD></TR>"
                               end if
                               ''end
        ...
        <%
            ...
            response.write "<TD ALIGN=CENTER WIDTH=""25%"" CLASS=Medium style=""background-color: RED"">"
            if Admin_Access = 2 and isblank(rs("Submitted_By")) then
              response.write "&nbsp;"
            elseif Admin_Access = 2 and CLng(rs("Submitted_By")) <> CLng(Admin_ID) then
              response.write "&nbsp;"
...
//Added on 17th Aug 2010, to hold Checkbox value in hidden parameter
function setCDN_RequiredInHidd(val)
{
    var cdnReq = document.getElementById("CDN_Required");
    if (cdnReq != null)
    {
       
        document.getElementById("hidCDN_Required").value = cdnReq.checked;
...
<script language="Javascript">
function openLocalizeWindow(val) {
     document.forms[0].action = "/sw-administrator/SW-PCAT_FNET_SAVE_ProductMultilingual.asp?ID="+ val;
