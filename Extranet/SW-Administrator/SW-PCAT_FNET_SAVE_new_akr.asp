...
'response.write oFileUpEE.form("Content_Language")
...
if (oFileUpEE.form("PCat") >= 0 and optiontext=false) then
		...
		if isblank(varNav_Clone) and isblank(varNav_Duplicate) then 

					Dim Oprn  
					Dim calendarId
		
					if not isblank(varNav_Update) then
						...
						elseif not isblank(varNav_Delete) then
											Oprn = "D"
						end if
					...
					if ((trim(PcatSaveFilePath) <> "" or trim(oFileUpEE.form("URLLink")) <> "" or trim(oFileUpEE.form("File_Existing")) <> "") and trim(Oprn) <> "D")  then
									...
									'Code added for gold silo aggregation(gold options) -- by zensar(11/17/08)
									    SubGroups = varSubGroups & ", "	              
									if IsObject(varSubGroups)=true then
											for each oSubItem in varSubGroups
													SubGroups = Subgroups & oSubItem.Value & ", "
											next
									else
											SubGroups= Subgroups & varSubGroups  
									end if    
									...
									if not isblank(oFileUpEE.form("URLLink")) then
											...
											if (Site_ID = 82) then
											    if  (Instr(1,LCase(oFileUpEE.form("URLLink")),"http://") = 0 and Instr(1,LCase(oFileUpEE.form("URLLink")),"https://") = 0) then
											        ...
											        PcatSaveFilePath =Replace(replace(ucase(ReplaceQuote(LCase(oFileUpEE.form("URLLink")))),ucase("/virtual_demo/fnet/"),""),"&","%26") 
											    else
											        'RI# Fnet 1008 gpd
											        'PcatSaveFilePath = oFileUpEE.form("URLLink")
											        PcatSaveFilePath = Replace(oFileUpEE.form("URLLink"),"&","%26") 
											    end if
											else
											    PcatSaveFilePath = replace(ucase(ReplaceQuote(LCase(oFileUpEE.form("URLLink")))),ucase("/virtual_demo/fnet/"),"")
											end if
									...
									' Country
									if Not isblank(varCountry) and instr(1,varCountry_Reset,"none") = 0 then
											if instr(1,varCountry_Reset,"0") > 0 then
...
sub Showerror(errordesc)
  ...
  .write "Error while adding\updating1 the records in Pcatalog<br><br>"
